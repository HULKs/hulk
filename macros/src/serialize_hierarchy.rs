...
pub fn process_serialize_hierarchy_implementation(
    ...
    let expanded = quote! {
        impl #impl_generics crate::framework::SerializeHierarchy for #name #ty_generics #where_clause {
            ...
            fn get_hierarchy() -> crate::framework::HierarchyType {
                ...
                crate::framework::HierarchyType::Struct {
                    fields,
                }
            }
        }
...
fn generate_hierarchy_insertions(fields: &Fields) -> Vec<TokenStream> {
    fields
        ...
        .filter_map(|field| {
            ...
            let code = if is_leaf {
                quote! {
                    fields.insert(#pattern.to_string(), crate::framework::HierarchyType::GenericStruct)
                }
            } else {
