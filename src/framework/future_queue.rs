...
impl<T> Consumer<T> {
    pub fn consume(&self, now: SystemTime) -> (Vec<Data<T>>, Option<SystemTime>) {
        let mut slots = self.slots.lock();
        ...
        let first_empty_data = slots
            ...
            .unwrap_or(slots.len());
        let finished = slots.drain(..first_empty_data).map(Data::from).collect();
