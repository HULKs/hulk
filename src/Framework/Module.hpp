...
public:
  ...
  /**
   * @brief runCycle is called every time some action has to be taken
   */
...
protected:
  ...
  /**
   * @brief robotMetrics provides access to the robot metrics provided by the robotInterface
   * @return the robot metrics
   */
  const RobotMetrics& robotMetrics() const
  {
    return robotInterface_.getRobotMetrics();
  }
  /**
   * @brief forwardKinematics provides access to the forward kinematics provided by the
   * robotInterface
   * @return the forward kinematics
   */
  const ForwardKinematics& forwardKinematics() const
  {
    return robotInterface_.getRobotMetrics().forwardKinematics();
  }
  /**
   * @brief inverseKinematics provides access to the inverse kinematics provided by the
   * robotInterface
   * @return the inverse kinematics
   */
  const InverseKinematics& inverseKinematics() const
  {
    return robotInterface_.getRobotMetrics().inverseKinematics();
  }
  /**
   * @brief com provides access to the center of mass provided by the robotInterface
   * @return the center of mass
   */
  const Com& com() const
  {
    return robotInterface_.getRobotMetrics().com();
  }
...
public:
  ...
  static ModuleFactoryBase* begin__;
};
...
public:
  ...
  ModuleFactory()
  {
    ModuleFactoryBase<T2>::next = ModuleFactoryBase<T2>::begin__;
    ModuleFactoryBase<T2>::begin__ = this;
  }
  /**
...
public:
   ...
   */
  explicit Module(const ModuleManagerInterface& manager)
    : ModuleBase{manager, T::name__}
  {
  ...
  void runCycle() final
  {
    ...
    {
      database_.reset(production);
    }
    cycle();
    for (auto& production : productions_)
    {
      auto& realProduction = database_.get(production);
  ...
  /**
   * @brief cycle is called every time some action has to be taken
   */
...
private:
  ...
  static ModuleFactory<T, T2> factory__;
};

template <typename T, typename T2>
ModuleFactory<T, T2> Module<T, T2>::factory__;
...
public:
   ...
   */
  explicit Dependency(ModuleBase& module)
...
public:
   ...
   */
  explicit Reference(ModuleBase& module)
    : data_(module.database_.get<T>())
  {
  }
  /**
...
public:
   ...
   * subscribed)
   */
  explicit Production(ModuleBase& module, bool autoUpdateDebug = true)
    : data_(module.database_.get<T>())
  {
...
public:
   ...
   * @param callback is called every time the value is changed
   */
    ...
    , callback_(std::move(callback))
  {
    ...
    if (callback_)
    {
      module.configuration_.registerCallback(
          module.mount_, key, boost::bind(&Parameter<T>::onUpdate, this, boost::placeholders::_1));
    }
  }
  /**
...
public:
  ...
  T& operator()()
  {
    ...
    isFirstValue_ = condition_();
    return isFirstValue_ ? value.first : value.second;
  }
...
private:
  /// condition to use the first value
  std::function<bool()> condition_;
  /// keeps track if the first value is in use
  mutable bool isFirstValue_{};
};

template <typename T>
ModuleFactoryBase<T>* ModuleFactoryBase<T>::begin__ = nullptr;
