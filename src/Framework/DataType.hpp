...
public:
   ...
   * @param destination to copy the data to
   */
  virtual void copy(DataTypeBase* destination) const = 0;
  /**
...
public:
   ...
   */
  DataTypeBase* copy() const override
  {
    return new Derived(static_cast<const Derived&>(*this));
  }
  /**
   * @brief copy creates a copy of a data type at a specified location
   * @param destination of the copy operation
   */
  void copy(DataTypeBase* destination) const override
  {
    *static_cast<Derived*>(destination) = Derived(static_cast<const Derived&>(*this));
  }
  /**
