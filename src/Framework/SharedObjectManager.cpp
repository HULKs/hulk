...
SharedObjectManager::SharedObjectManager(Debug& debug, Configuration& config,
...
{
  Log<M_TUHHSDK>(LogLevel::INFO) << "Initializing shared objects";
  ...
  config_.mount("tuhhSDK.moduleSetup",
                "moduleSetup_" + config_.get("tuhhSDK.autoload", "moduleSetup").asString() +
                    ".json",
  ...
  connectionChannels_.resize(numEdges);
  ...
  for (size_t numFirst = 0; numFirst < numVertices; numFirst++)
  {
    for (size_t numSecond = numFirst + 1; numSecond < numVertices; numSecond++)
    {
      ...
      auto& d = connectionChannels_[assignedChannel];
  ...
  for (; itS != itE; itS++)
  {
    ...
    tData.loglevel = Log<M_TUHHSDK>::getLogLevel(logl);
    tData.debug = &debug_;
    tData.configuration = &config_;
    tData.robotInterface = &robotInterface_;
    Log<M_TUHHSDK>(LogLevel::INFO) << "Loading sharedObject\"" << sharedObject << "\" ...";

    try
    {
      loadedSharedObjects_.emplace_back(sharedObject, tData);
      Log<M_TUHHSDK>(LogLevel::INFO) << "... Success";
    }
...
void SharedObjectManager::checkAllRequestedDataTypes()
{
  ...
  {
    Log<M_TUHHSDK>(LogLevel::ERROR) << "Unresolved dependencies:";
    for (auto& unresolvedDependency : unresolvedDependencies)
    {
      Log<M_TUHHSDK>(LogLevel::ERROR) << unresolvedDependency.name();
    }
