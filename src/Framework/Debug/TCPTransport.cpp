...
void TCPTransport::Session::readHeader()
{
  ...
  boost::asio::async_read(
      ...
      [this, self](boost::system::error_code error, std::size_t length) {
        ...
        Log<M_TUHHSDK>(LogLevel::DEBUG) << "TCPTransport: received header";
...
void TCPTransport::Session::readBody()
{
  ...
  boost::asio::async_read(
      ...
      [this, self](boost::system::error_code error, std::size_t /*length*/) {
        ...
        {
          Log<M_TUHHSDK>(LogLevel::ERROR)
              << "TCPTransport: error while receiving body, disconnecting. Error: "
              << error.message();
          server_.disconnected(self);
          return;
        }

        Log<M_TUHHSDK>(LogLevel::DEBUG) << "TCPTransport: received body";

        std::string body(bodyBuffer_.begin(), bodyBuffer_.end());
        parseBody(body);

        Log<M_TUHHSDK>(LogLevel::DEBUG) << "TCPTransport: Parsed Body";
    ...
    try
    {
      ...
      boost::asio::async_write(
          ...
          [this, self](boost::system::error_code error, std::size_t /*length*/) {
            ...
            Log<M_TUHHSDK>(LogLevel::DEBUG) << "TCPTransport: sent Updates.";
          });
#else
#ifdef ITTNOTIFY_FOUND
      __itt_event_start(this->eventTransmission_);
...
void TCPTransport::Impl::startBackgroundThread()
{
  backgroundThread_ = std::make_shared<std::thread>([this]() {
    Log<M_TUHHSDK>(LogLevel::INFO) << "TCPTransport: Starting background thread.";
    ioService_.run();
    Log<M_TUHHSDK>(LogLevel::INFO) << "TCPTransport: background thread terminated.";
  });
}

void TCPTransport::Impl::startAccept()
{
  Log<M_TUHHSDK>(LogLevel::INFO) << "TCPTransport: Waiting for connection";
