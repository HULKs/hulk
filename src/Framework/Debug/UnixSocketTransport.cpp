...
#include "Framework/Debug/DebugMessageFormat.h"
...
void UnixSocketTransport::Session::readHeader()
{
  ...
  boost::asio::async_read(
      ...
      [this, self](boost::system::error_code error, std::size_t length) {
        ...
        Log<M_TUHHSDK>(LogLevel::DEBUG) << "UnixSocketTransport: received header";
...
void UnixSocketTransport::Session::readBody()
{
  ...
  boost::asio::async_read(
      ...
      [this, self](boost::system::error_code error, std::size_t /*length*/) {
        ...
        Log<M_TUHHSDK>(LogLevel::DEBUG) << "UnixSocketTransport: received body";

        std::string body(bodyBuffer_.begin(), bodyBuffer_.end());
        parseBody(body);

        Log<M_TUHHSDK>(LogLevel::DEBUG) << "UnixSocketTransport: Parsed Body";
...
void UnixSocketTransport::Session::parseBody(const std::string& body)
{
  ...
  {
    case DM_SUBSCRIBE: {
      Log<M_TUHHSDK>(LogLevel::DEBUG) << "DM_SUBSCRIBE-Message received: " + body;
      subscribe(body);
    }
    break;
    case DM_SUBSCRIBE_BULK: {
      Log<M_TUHHSDK>(LogLevel::DEBUG) << "DM_SUBSCRIBE_BULK-Message received: " << body;
      subscribeBulk(body);
    }
    break;
    case DM_UNSUBSCRIBE: {
      Log<M_TUHHSDK>(LogLevel::DEBUG) << "DM_UNSUBSCRIBE-Message received: " << body;
      unsubscribe(body);
    }
    break;
    case DM_REQUEST_LIST: {
      Log<M_TUHHSDK>(LogLevel::DEBUG) << "DM_REQUEST_LIST-Message received.";
      sendList_.store(true);
    }
    break;
    default:
      Log<M_TUHHSDK>(LogLevel::WARNING) << "Command is not to be implemented on the Server/Robot.";
      break;
  }
}
  ...
  for (const auto& debugSource : data)
  {
    ...
    if (debugMap == nullptr)
    {
      Log<M_TUHHSDK>(LogLevel::INFO)
          << "Unable to compute the complete keylist, waiting for next cycle.";
      continue;
    }
    ...
    for (const auto& dataEntry : debugMap->getDebugMap())
    {
      ...
      {
        DebugData dat{dataEntry.first, dataEntry.second.data.get(), dataEntry.second.updateTime};
        transmitList_[i] << dat;
      }
      else
      {
    ...
    if (canTransmit_.compare_exchange_weak(expected, false))
    {
      ...
      boost::asio::async_write(
          ...
          [this, self, hdr, json](boost::system::error_code error, std::size_t /*length*/) {
            ...
            Log<M_TUHHSDK>(LogLevel::DEBUG) << "UnixSocketTransport: sent List.";
          });
#else
#ifdef ITTNOTIFY_FOUND
      __itt_event_start(this->eventTransmission_);
...
void UnixSocketTransport::Session::transport()
{
  ...
  if (!canTransmit_.compare_exchange_weak(expected, false))
  {
    Log<M_TUHHSDK>(LogLevel::DEBUG) << "Could not transmit debug updates, already transmitting";
    return;
  }
  ...
  try
  {
    ...
    for (auto key = subscriptionList_.begin(); key != subscriptionList_.end(); ++key)
    {
      ...
      else
      {
        ...
        imageData.timestamp = std::chrono::duration_cast<std::chrono::nanoseconds>(
                                  debugMapEntry->updateTime.time_since_epoch())
                                  .count();
        imageData.width = img.size.x();
        imageData.height = img.size.y();
    ...
    try
    {
      ...
      boost::asio::async_write(
          ...
          [this, self](boost::system::error_code error, std::size_t /*length*/) {
            ...
            Log<M_TUHHSDK>(LogLevel::DEBUG) << "UnixSocketTransport: sent Updates.";
          });
#else
#ifdef ITTNOTIFY_FOUND
      __itt_event_start(this->eventTransmission_);
  ...
  {
    canTransmit_.store(true);
    Log<M_TUHHSDK>(LogLevel::ERROR) << "Exception transmitting debug updates: " << e.what();
  }
}
...
void UnixSocketTransport::Impl::startBackgroundThread()
{
  backgroundThread_ = std::make_shared<std::thread>([this]() {
    Log<M_TUHHSDK>(LogLevel::INFO) << "UnixSocketTransport: Starting background thread.";
    ioService_.run();
    Log<M_TUHHSDK>(LogLevel::INFO) << "UnixSocketTransport: background thread terminated.";
  });
}

void UnixSocketTransport::Impl::startAccept()
{
  Log<M_TUHHSDK>(LogLevel::INFO) << "UnixSocketTransport: Waiting for connection";
