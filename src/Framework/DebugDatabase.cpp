...
void DebugDatabase::DebugMap::playAudio(const std::string& key, const AudioSounds audioSound)
{
  assert(inUse_.load() == true &&
         "Map to update was not in use (at least we should mark this map as used)");

  // Inform the DebugDatabase about the requested sound
  debugDatabase_->requestedSounds_.push(audioSound);
  ...
  // Update the given key value pair.
  *(it->second.data) = Uni::Value(static_cast<int>(audioSound));
  it->second.updateTime = updateTime_;
...
void DebugDatabase::DebugMap::setUpdateTime(Clock::time_point updateTime)
{
  updateTime_ = updateTime;
}

Clock::time_point DebugDatabase::DebugMap::getUpdateTime() const
{
  return updateTime_;
}
...
{
  bool expected = false;
  currentlyTransportedMap_ = nextDebugMapToTransport_.load();
  if (currentlyTransportedMap_ == nullptr)
  {
    return nullptr;
  }

  if (!currentlyTransportedMap_->inUse_.compare_exchange_weak(expected, true))
  {
    // failed to lock the new map.
    currentlyTransportedMap_ = nullptr;
  }
  else if (currentlyTransportedMap_->updateTime_ <= currentTransportMapUpdateTime_)
  {
...
bool DebugDatabase::popLastRequestedSound(AudioSounds& audioSound) const
{
  return requestedSounds_.pop(audioSound);
}
