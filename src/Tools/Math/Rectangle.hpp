...
public:
   ...
   */
  explicit Rectangle(const Vector2<T>& a = Vector2<T>::Zero(),
                     const Vector2<T>& b = Vector2<T>::Zero())
  ...
  /**
   * @brief Get the Rectangle as YUV444 from YUV422 coordinates.
   */
  Rectangle<T> get444from422() const
  {
    Rectangle<T> converted444(topLeft, bottomRight);
    converted444.topLeft.x() *= 2;
    converted444.bottomRight.x() *= 2;
    return converted444;
  }

  /**
   * @brief Get the Rectangle as YUV422 from YUV444 coordinates.
   */
  Rectangle<T> get422from444() const
  {
    Rectangle<T> converted422(topLeft, bottomRight);
    converted422.topLeft.x() /= 2;
    converted422.bottomRight.x() /= 2;
    return converted422;
  }

  /**
   * @brief Convert this Rectangle from YUV422 to YUV444 coordinates.
   */
