...
public:
   ...
   */
  Velocity(float translation, float rotation = 0.f, bool usePercentage = true)
   ...
   * velocityComponents
   * @param maxVelocityComponents The maximum velocities in all three directions (x, y, alpha).
   * These are the maximum velocities that would be possible in each of the directions alone. The
  ...
  Velocity getAbsolute(const Pose& maxVelocityComponents) const
  {
    ...
    assert(maxVelocityComponents.x() >= 0.f && maxVelocityComponents.y() >= 0.f &&
           maxVelocityComponents.angle() >= 0.f);
    ...
    const float maxTranslationVelocity =
        std::max(maxVelocityComponents.x(), maxVelocityComponents.y());
    // transform to aboslute velocity while not changing direction
    return {translation.normalized() * maxTranslationVelocity,
            rotation * maxVelocityComponents.angle(), false};
  }
  Pose convertToPose() const
  {
