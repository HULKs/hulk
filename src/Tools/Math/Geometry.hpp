...
#include "Tools/Math/Plane.hpp"

namespace Geometry
{
  /**
   * @brief calculates a point projected onto a line
   * @param p the point to project
   * @param line the line to project on
   * @return the point projected on the line
   */
  template <typename T>
  inline Vector2<T> projectPointOnLine(const Vector2<T>& p, const Line<T>& line)
  {
    const Vector2<T> l = line.p2 - line.p1;
    const Vector2<T> e = p - line.p1;
    const T dot = l.dot(e);
    const T lenSquared = l.squaredNorm();
    assert(lenSquared > 0);
    return {line.p1.x() + dot * l.x() / lenSquared, line.p1.y() + dot * l.y() / lenSquared};
  }
   ...
   */
  template <typename T>
  inline Vector2<T> getPointToLineVector(const Vector2<T>& p, const Line<T>& line)
  {
    const Vector2<T> projected = projectPointOnLine(p, line);
    return projected - p;
  }
  ...
  /**
   * @brief computes a point at a specified distance to both given points a and b
   * @param a the first point
   * @param b the second point
   * @param distance the distance from both a and b to the calculated point
   * @param left whether to return the point left of the vector from a to b or the right one
   * @return the calculated point
   */
  template <typename T>
  inline Vector2<T> getEquidistantPoint(const Vector2<T> a, const Vector2<T> b, T distance,
                                        const bool left)
  {
    const Vector2<T> mid = (b + a) / static_cast<T>(2);
    const Vector2<T> aToB = b - a;
    const auto baseLengthSquared = aToB.squaredNorm() / static_cast<T>(4);
    const auto h = std::sqrt(distance * distance - baseLengthSquared);
    return mid + Vector2<T>{-aToB.y(), aToB.x()}.normalized() * (left ? h : -h);
  }
                        ...
                        std::pair<Vector2<T>, Vector2<T>>& intersection)
  {
    ...
    const T center1ToIntersectionLine(
        ...
        (2 * distance));
    const T intersectionLineHeight(std::sqrt(
        circle1.radius * circle1.radius - center1ToIntersectionLine * center1ToIntersectionLine));
  ...
  /**
   * @brief getTangentPointsOfCircleWithoutChecks calculates the tangent points of two tangents
   * between a point and a circle, it assumes that the point does not lie within the circle
   * @param point the point
   * @param circle the circle
   * @return the pair of tangent points
   */
  inline std::pair<Vector2f, Vector2f> getTangentsOfCircleWithoutChecks(const Vector2f& point,
                                                                        const Circle<float>& circle)
  {
    assert((circle.center - point).squaredNorm() + std::numeric_limits<float>::epsilon() >
               circle.radius * circle.radius &&
           "getTangentsOfCircleWithoutChecks: Point should be outside the circle");
    const float f = point.x() - circle.center.x();
    const float g = point.y() - circle.center.y();

    const float radiusSquared = circle.radius * circle.radius;
    const float fgSquared = f * f + g * g;
    const float c = fgSquared - radiusSquared;
    const float sqrtC = std::sqrt(c);
    const float x1 =
        (radiusSquared * f + circle.radius * g * sqrtC) / fgSquared + circle.center.x();
    const float x2 =
        (radiusSquared * f - circle.radius * g * sqrtC) / fgSquared + circle.center.x();
    const float y1 =
        (radiusSquared * g - circle.radius * f * sqrtC) / fgSquared + circle.center.y();
    const float y2 =
        (radiusSquared * g + circle.radius * f * sqrtC) / fgSquared + circle.center.y();
    return std::pair<Vector2f, Vector2f>{{x1, y1}, {x2, y2}};
  }

  /**
   * @brief getExteriorTangentsBetweenCircles calculates the exterior tangents between two circles
   *        DO NOT CALL WITH CIRCLES THAT INTERSECT, make sure this cant happen before calling
   *        (see getTangentsBetweenCircles)
   * @param circle1 the first circle
   * @param circle2 the second circle
   * @param[out] the calculated tangents always places 2 tangents in the vector,
   *             the first points of the tangents are on circle1
   *             the second points of the tangents are on circle2
   */
  inline void getExteriorTangentsBetweenCircles(const Circle<float>& circle1,
                                                const Circle<float>& circle2,
                                                std::vector<Line<float>>& tangents)
  {
    if (std::abs(circle1.radius - circle2.radius) < std::numeric_limits<float>::epsilon())
    {
      // If the circles are the same size calculation is much easier
      const Vector2f connectingLine = circle1.center - circle2.center;
      const Vector2f shiftVector =
          Vector2f{connectingLine.y(), -connectingLine.x()}.normalized() * circle1.radius;
      tangents.emplace_back(circle1.center + shiftVector, circle2.center + shiftVector);
      tangents.emplace_back(circle1.center - shiftVector, circle2.center - shiftVector);
      return;
    }
    const auto* largerCircle = &circle1;
    const auto* smallerCircle = &circle2;
    bool swapped = false;

    // If the assumption above didn't hold, swap the pointers
    if (circle1.radius < circle2.radius)
    {
      std::swap(smallerCircle, largerCircle);
      swapped = true;
    }

    // Take the large circle and shrink it by the radius of the smaller one
    // then calculate the tangent points for the simplified problem
    auto shrunkenRadius = largerCircle->radius - smallerCircle->radius;
    const std::pair<Vector2f, Vector2f> tangentPoints = getTangentsOfCircleWithoutChecks(
        smallerCircle->center, {largerCircle->center, shrunkenRadius});

    // Shift the calculated tangent points out again
    const Vector2f shiftVec1 =
        (tangentPoints.first - largerCircle->center).normalized() * smallerCircle->radius;
    const Vector2f shiftVec2 =
        (tangentPoints.second - largerCircle->center).normalized() * smallerCircle->radius;
    if (swapped)
    {
      // circle2 was the larger one
      tangents.emplace_back(smallerCircle->center + shiftVec1, tangentPoints.first + shiftVec1);
      tangents.emplace_back(smallerCircle->center + shiftVec2, tangentPoints.second + shiftVec2);
      return;
    }
    // circle1 was the larger one
    tangents.emplace_back(tangentPoints.first + shiftVec1, smallerCircle->center + shiftVec1);
    tangents.emplace_back(tangentPoints.second + shiftVec2, smallerCircle->center + shiftVec2);
  }

  /**
   * @brief getInteriorTangentsBetweenCircles calculates the interior tangents between two circles
   *        DO NOT CALL WITH CIRCLES THAT INTERSECT, make sure this cant happen before calling
   *        (see getTangentsBetweenCircles)
   * @param circle1 the first circle
   * @param circle2 the second circle
   * @param[out] the calculated tangents always places 2 tangents in the vector
   */
  inline void getInteriorTangentsBetweenCircles(const Circle<float>& circle1,
                                                const Circle<float>& circle2,
                                                std::vector<Line<float>>& tangents)
  {
    if (std::abs(circle1.radius - circle2.radius) < std::numeric_limits<float>::epsilon())
    {
      // If the circles are the same size calculation is much easier
      // simply use the middle point of the connection line of the circle centers as the
      // point where the tangents cross and then do a point to circle tangent search twice
      // connecting the points to lines in the end
      const Vector2f connectingLine = circle1.center - circle2.center;
      const Vector2f crossingPoint = (connectingLine / 2.f) + circle2.center;

      // Do point to circle tangents for both circles from the middle point
      const std::pair<Vector2f, Vector2f> tangentPointsCircle1 =
          getTangentsOfCircleWithoutChecks(crossingPoint, circle1);
      const std::pair<Vector2f, Vector2f> tangentPointsCircle2 =
          getTangentsOfCircleWithoutChecks(crossingPoint, circle2);

      // Create lines from tangent points
      tangents.emplace_back(tangentPointsCircle1.first, tangentPointsCircle2.first);
      tangents.emplace_back(tangentPointsCircle1.second, tangentPointsCircle2.second);
      return;
    }
    // crossing point of tangents calculation has to be done a bit differently than in
    // case of same sized circles calculations can be found at:
    // http://www.ambrsoft.com/TrigoCalc/Circles2/Circles2Tangent_.htm

    // Calculate the crossing point
    const auto radiusSum = circle1.radius + circle2.radius;
    const auto crossX =
        (circle2.center.x() * circle1.radius + circle1.center.x() * circle2.radius) / radiusSum;
    const auto crossY =
        (circle2.center.y() * circle1.radius + circle1.center.y() * circle2.radius) / radiusSum;
    const Vector2f crossingPoint{crossX, crossY};
  ...
  /**
   * @brief getTangentPointsOfCircle calculates the tangents between two circles
   *        see http://www.ambrsoft.com/TrigoCalc/Circles2/Circles2Tangent_.htm
   * @param circle1 the first circle
   * @param circle2 the second circle
   * @param[out] the calculated tangents, can be between 0 and 4 lines, needs to be empty
   * @return bool whether the calculation worked or not
   */
  inline void getTangentsBetweenCircles(const Circle<float>& circle1, const Circle<float>& circle2,
                                        std::vector<Line<float>>& tangents)
  {
    tangents.reserve(4);
    const auto distanceSquared = (circle1.center - circle2.center).squaredNorm();
    const auto radiusDifferenceSquared =
        (circle1.radius - circle2.radius) * (circle1.radius - circle2.radius);
    const auto radiusSumSquared =
        (circle1.radius + circle2.radius) * (circle1.radius + circle2.radius);

    if (distanceSquared <= radiusDifferenceSquared)
    {
      // one circle is completely within the other, ignores the case where circles touch
      // return empty vector
      return;
    }
    if (radiusDifferenceSquared < distanceSquared && distanceSquared <= radiusSumSquared)
    {
      // Circles overlap, there are only 2 outer tangents, ignore the touching case
      getExteriorTangentsBetweenCircles(circle1, circle2, tangents);
      return;
    }
    // Circles don't touch at all, there are going to be 4 tangents
    getExteriorTangentsBetweenCircles(circle1, circle2, tangents);
    getInteriorTangentsBetweenCircles(circle1, circle2, tangents);
  }

  /**
   * @brief Checks whether the subject vector lies left of a reference vector
   * @param reference the reference vector
   * @param subject the subject vector
   * @return bool whether subject vector lies left of reference vector
   */
  template <typename T>
  inline bool isLeftOf(const Vector2<T>& reference, const Vector2<T>& subject)
  {
    // Rotate reference by 90 degrees ccw
    Vector2<T> temp(-reference.y(), reference.x());
    return temp.dot(subject) > 0.f;
  }

  /**
   * @brief getBlockedArcs finds out which arcs are blocked for intersecting circles
   * @param circle1 the first circle
   * @param circle2 the second circle
   * @return a vector of blocked arcs, if it is empty the circles don't intersect or only
   *         one intersection point intersects. All arcs are defined as counterclock wise.
   *         the first arc belongs to circle1 and the second to circle2.
   */
  inline std::vector<Arc<float>> getBlockedArcs(const Circle<float>& circle1,
                                                const Circle<float>& circle2)
  {
    std::vector<Arc<float>> arcs;
    arcs.reserve(2);

    // Calculate intersection points
    std::pair<Vector2f, Vector2f> intersections;
    CircleIntersectionType type = getCircleIntersection(circle1, circle2, intersections);

    // Only consider situations where there are two intersections
    if (type != CircleIntersectionType::TWO_INTERSECTIONS)
    {
      return arcs;
    }

    // For now we need to guess which is the start and which is the end of the arcs
    // This will be adjusted further down
    Vector2f startIntersection1 = intersections.first;
    Vector2f endIntersection1 = intersections.second;
    Vector2f startIntersection2 = intersections.first;
    Vector2f endIntersection2 = intersections.second;
    const Vector2f startToIntersection1 = intersections.first - circle1.center;
    const Vector2f endToIntersection1 = intersections.second - circle1.center;
    const Vector2f startToIntersection2 = intersections.first - circle2.center;
    const Vector2f endToIntersection2 = intersections.second - circle2.center;

    // We now know the intersection points but we need to find out which of the
    // two arcs is blocked. First we calculate the vectors between the circles' centers
    // and the middlepoint of the line connecting the two intersection points
    const Vector2f middlePoint =
        intersections.second + (intersections.first - intersections.second) / 2.f;
    Vector2f toMiddle1 = middlePoint - circle1.center;
    Vector2f toMiddle2 = middlePoint - circle2.center;

    // If the dot product of these vectors is negative the two circles intersect like
    // the two circles in an 8 (not touching but actually intersecting).
    // If it is positive a larger circle contains most of the smaller one.
    // If it is zero one of the vectors was zero which means that the intersection points
    // are right in the middle of one of the two circles which has to be handled specifically.
    float dot = toMiddle1.dot(toMiddle2);
    if (dot < 0.f)
    {
      // Swap start and end if necessary
      if (isLeftOf(endToIntersection1, startToIntersection1))
      {
        std::swap(endIntersection1, startIntersection1);
      }
      if (isLeftOf(endToIntersection2, startToIntersection2))
      {
        std::swap(endIntersection2, startIntersection2);
      }

      // Create blocked arcs
      arcs.emplace_back(circle1, startIntersection1, endIntersection1, false);
      arcs.emplace_back(circle2, startIntersection2, endIntersection2, false);
    }
    else if (dot == 0.f)
    {
      // Find the zero and non zero vector
      Vector2f zero = toMiddle1;
      Vector2f nonZero = toMiddle2;
      if (toMiddle2.x() == 0.f && toMiddle2.y() == 0.f)
      {
        std::swap(zero, nonZero);
      }

      // Calculate the vector from zero circle to non zero circle, this vector has to be left of
      // start and right of end for the zero vector and the other way around for the non zero one
      Vector2f zeroToNonZero = nonZero;
      if (zero == toMiddle1)
      {
        if (!isLeftOf(startToIntersection1, zeroToNonZero) ||
            isLeftOf(endToIntersection1, zeroToNonZero))
        {
          std::swap(startIntersection1, endIntersection1);
        }
        if (!isLeftOf(startToIntersection2, toMiddle2) || isLeftOf(endToIntersection2, toMiddle2))
        {
          std::swap(startIntersection2, endIntersection2);
        }
      }
      else
      {
        if (isLeftOf(startToIntersection1, toMiddle1) || isLeftOf(endToIntersection1, toMiddle1))
        {
          std::swap(startIntersection1, endIntersection1);
        }
        if (!isLeftOf(startToIntersection2, zeroToNonZero) ||
            isLeftOf(endToIntersection2, zeroToNonZero))
        {
          std::swap(startIntersection2, endIntersection2);
        }
      }
    ...
    {
      // Need to find the smaller circle as this defines which arc is blocked for which circle
      const Circle<float>* smallerCircle = &circle1;
      if (circle1.radius > circle2.radius)
      {
      ...
      // Swap start and end if necessary
      if (smallerCircle->center == circle1.center)
      {
        if (isLeftOf(startToIntersection1, endToIntersection1))
        {
          std::swap(startIntersection1, endIntersection1);
        }
        if (!isLeftOf(startToIntersection2, endToIntersection2))
        {
          std::swap(startIntersection2, endIntersection2);
        }
      }
  ...
  /**
   * @brief Checks if the line has an intersection with the given circle
   *        Note: Touching is not considered an intersection
   * @param line The line to check for intersection with the circle
   * @param circle The circle to check for intersection with
   * @param epsilon The epsilon for numerical stability
   * @return Whether line intersects with circle
   */
  template <typename T>
  bool hasIntersection(const Line<T>& line, const Circle<T>& circle,
                       T epsilon = std::numeric_limits<T>::epsilon())
  {
    const auto distance = getSquaredLineSegmentDistance(line, circle.center);
    return circle.radius * circle.radius - distance > epsilon;
  }
