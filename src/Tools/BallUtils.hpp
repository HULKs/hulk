...
namespace BallUtils
{
   ...
   * @param distanceToBallX the desired distance between the robot origin and the ball in x
   * direction [m]
   ...
   * necessary since turn kicks don't result in ball velocities parallel to the torso's x-axis
                       ...
                       const float distanceToBallY = 0.05f, const float kickDirectionAngle = 0.f)
  {
    ...
    const Vector2f footSelect(static_cast<float>(lastSign) * footSelect1.y(),
                              static_cast<float>(-lastSign) * footSelect1.x());
                       ...
                       const Vector2f& ballSource, const Vector2f& ballTarget)
  {
    ...
    return kickPose(ballSource, ballTarget, inWalkKick.distanceToBallX, kickFootSign, forceSign,
                    inWalkKick.distanceToBallY,
                    static_cast<float>(kickFootSign) * inWalkKick.kickDirectionAngle);
  }
                           ...
                           const KickFoot forceKickFoot = KickFoot::NONE)
  {
    ...
    const auto kickableBallAgeMax = 1s;
    if (ballState.found && ballState.age < kickableBallAgeMax)
    {
      ...
      bool correctDirection = Angle::angleDiff(0, kickPose.angle()) < angleToBall;

      if (nearLeft && correctDirection && forceKickFoot != KickFoot::RIGHT)
      {
        return Kickable::LEFT;
      }
      if (nearRight && correctDirection && forceKickFoot != KickFoot::LEFT)
      {
