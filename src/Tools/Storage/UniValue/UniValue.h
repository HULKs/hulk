...
namespace Uni
{
  ...
  public:
    ...
    /**
     * @brief asFlout() returns value converted to float, by retreiving the value as double
     * @throws std::runtime_error if value is not convertible to double
...
inline void operator>>(const Uni::Value& in, std::uint8_t& out)
{
  out = static_cast<std::uint8_t>(in.asInt32());
}

inline void operator<<(Uni::Value& out, const std::uint8_t in)
{
  out = Uni::Value(static_cast<std::int32_t>(in));
}
...
template <class Rep, class Period = std::chrono::seconds>
inline void operator<<(Uni::Value& out, const std::chrono::duration<Rep, Period>& duration)
{
  out = Uni::Value(duration.count());
}

template <class Rep, class Period = std::chrono::seconds>
inline void operator>>(const Uni::Value& in, std::chrono::duration<Rep, Period>& duration)
{
  static_assert(std::chrono::treat_as_floating_point_v<Rep>);
  duration = std::chrono::duration<Rep, Period>{in.asDouble()};
}

template <class Clock, class Duration = typename Clock::duration>
inline void operator<<(Uni::Value& out, const std::chrono::time_point<Clock, Duration>& timePoint)
{
  out = Uni::Value{timePoint.time_since_epoch().count()};
}

template <class Clock, class Duration = typename Clock::duration>
inline void operator>>(const Uni::Value& in, std::chrono::time_point<Clock, Duration>& timePoint)
{
  static_assert(std::chrono::treat_as_floating_point_v<typename Clock::rep>);
  timePoint = std::chrono::time_point<Clock, Duration>{
      std::chrono::duration<typename Clock::rep, typename Clock::period>{in.asDouble()}};
}

#include "Tools/Storage/UniValue/EigenStreaming.hpp"
