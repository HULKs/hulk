...
#include "Tools/Storage/XPM/X11rgb.h"
#include "Tools/Storage/XPM/XPMImages.h"
...
XPMImage XPMImage::loadXPMImage(const char** rawXPM, const char* transparencyChar)
{
  ...
  for (unsigned int i = 0; i < numberOfColors; ++i)
  {
    ...
    if (colorPair[2].str().compare(0, 1, "#") == 0) // check for hex value
    {
      ...
      colorMap[colorPair[1].str()] = Color(RGBColor(rgb[0], rgb[1], rgb[2]));
    }
    else // x11 color
    {
  ...
  for (unsigned int y = 0; y < height; ++y)
  {
    for (unsigned int x = 0; x < width; ++x)
    {
      ...
      char subStr[5];
      memcpy(subStr, &rawXPM[y + 1 + numberOfColors][x * charPerPixel],
             charPerPixel * sizeof(char));
      subStr[charPerPixel] = '\0';
      if (transparencyChar && *subStr == *transparencyChar)
      {
        image.data[y * width + x] = Color::TRANSPARENT;
      }
      else
      {
        image.data[y * width + x] = colorMap[subStr];
      }
    }
  }
...
void XPMImage::init()
{
  ...
  for (unsigned int i = 0; i < sizeof(x11RGB) / 8; ++i)
  {
    ...
    x11Colors_[colorValues[4].str()] =
        Color(RGBColor(std::stoi(colorValues[1].str()), std::stoi(colorValues[2].str()),
                       std::stoi(colorValues[3].str())));
  }
