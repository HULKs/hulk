...
#include "Brain/WhistleDetection/WhistleDetection.hpp"
...
void WhistleDetection::cycle()
{
  ...
  if (!recordData_->valid || recordData_->samples[channel_()].empty())
  {
    if (robotInterface().getAudio().getAudioProperties().recordingSupported)
    {
      Log<M_BRAIN>(LogLevel::ERROR)
          << "Platform supports audio recording, but record data was invalid";
      assert(false);
    }
    return;
  }

  for (SampleRingBufferIt it = recordData_->cycleStartIterators[channel_()] + 1;
       it != recordData_->samples[channel_()].end(); ++it)
  {
    fftBuffer_.push_back(*it);
...
bool WhistleDetection::fftBufferContainsWhistle()
{
  ...
  // resolution
  double freqResolution = AudioInterface::captureSamplingRate / fftBufferSize_;
