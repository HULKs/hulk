...
private:
  ...
  struct BallMode
  {
    ...
    BallMode(const BallFilter& ballFilter)
      : maxBufferSize{static_cast<size_t>(1.f / ballFilter.cycleInfo_->cycleTime.count())}
    {
    }

    float getPerceptsPerSecond(const Clock::duration cycleTime) const
    {
      ...
      static_assert(std::is_same_v<Clock::duration::period, std::chrono::seconds::period>);
      return static_cast<float>(numValidPercepts) /
             (static_cast<float>(currentSize) * cycleTime.count());
    }
  ...
  Clock::time_point lastPrediction_;
  /// the time when the ball was lost
  Clock::time_point timeWhenBallLost_;
