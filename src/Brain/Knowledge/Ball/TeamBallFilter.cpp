...
#include "Brain/Knowledge/Ball/TeamBallFilter.hpp"


TeamBallFilter::TeamBallFilter(const ModuleManagerInterface& manager)
  : Module{manager} // TODO: The values in the config file are currently guessed.
...
void TeamBallFilter::addBallToBuffer(const unsigned int playerNumber, const Pose& pose,
                                     ...
                                     const Vector2f& relBallVelocity,
...
void TeamBallFilter::updateBallBuffer()
{
  ...
  for (const auto& player : teamPlayers_->players)
  {
    ...
    if (!player.isPoseValid || player.penalized ||
        cycleInfo_->getAbsoluteTimeDifference(player.timeWhenBallWasSeen) > maxAddAge_() ||
        player.ballVelocity.norm() > maxBallVelocity_() ||
        cycleInfo_->getAbsoluteTimeDifference(player.timestampLastJumped) <
            minWaitAfterJumpToAddBall_())
    {
      continue;
    }
  ...
  ballBuffer_.erase(std::remove_if(ballBuffer_.begin(), ballBuffer_.end(),
                                   [this](const TeamPlayerBall& ball) {
                                     return cycleInfo_->getAbsoluteTimeDifference(
                                                ball.timeLastSeen) > minRemoveAge_();
                                   }),
                    ballBuffer_.end());
}
...
void TeamBallFilter::cycle()
{
    ...
    if (!allBallClusters.empty())
    {
      ...
      std::sort(allBallClusters.begin(), allBallClusters.end(),
                [](const BallCluster& a, const BallCluster& b) {
                  ...
                  return (a.balls.size() > b.balls.size() ||
                          (a.balls.size() == b.balls.size() && a.timeFirstSeen < b.timeFirstSeen));
                });
          ...
          (teamBallModel_->found || (teamBallModel_->seen && !ballState_->found)))
      {
        ...
        for (auto& bestBall : bestCluster.balls)
        {
          ...
          {
            teamBallModel_->absPosition = bestBall->ball.position;
            teamBallModel_->relPosition = robotPosition_->fieldToRobot(bestBall->ball.position);
          ...
          if (bestBall->distance < minDistance)
          {
      ...
      {
        teamBallModel_->absPosition = robotPosition_->robotToField(ballState_->position);
        teamBallModel_->relPosition = ballState_->position;
      ...
      gameControllerState_->gameState == GameState::READY)
  {
    ...
    if (gameControllerState_->gamePhase == GamePhase::PENALTYSHOOT)
    {
      ...
      teamBallModel_->absPosition = Vector2f(
          (fieldDimensions_->fieldLength * 0.5f - fieldDimensions_->fieldPenaltyMarkerDistance) *
              pmSign,
          0.f);
      teamBallModel_->relPosition = robotPosition_->fieldToRobot(teamBallModel_->absPosition);
    }
    else
    {
      teamBallModel_->absPosition = Vector2f::Zero();
      teamBallModel_->relPosition = Vector2f::Zero();
    }
  ...
  else
  {
    teamBallModel_->insideField =
        (teamBallModel_->ballType == TeamBallModel::BallType::NONE) ||
        fieldDimensions_->isInsideField(teamBallModel_->absPosition, insideFieldTolerance_());
  }
