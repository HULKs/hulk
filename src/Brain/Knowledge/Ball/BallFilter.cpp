#include "Brain/Knowledge/Ball/BallFilter.hpp"
...
BallFilter::BallFilter(const ModuleManagerInterface& manager)
  ...
  , relativeMovingThreshold_(
        *this, "relativeMovingThresholdPSOPair", [] {}, [&]() { return !isPenaltyKeeper(); })
  , restingErrorLowPassAlpha_(*this, "restingErrorLowPassAlpha", [] {})
  , movingErrorLowPassAlpha_(*this, "movingErrorLowPassAlpha", [] {})
  , maxRestingError_(
        *this, "maxRestingErrorPSOPair", [] {}, [&]() { return !isPenaltyKeeper(); })
  , numOfRestingDeccelerationSteps_(
        *this, "numOfRestingDeccelerationStepsPSOPair", [] {}, [&]() { return !isPenaltyKeeper(); })
...
void BallFilter::cycle()
{
  ...
  else
  {
    ...
    ballState_->age = cycleInfo_->getAbsoluteTimeDifference(bestMode_->lastUpdate);
...
void BallFilter::predict()
{
  ...
  ballModes_.erase(std::remove_if(ballModes_.begin(), ballModes_.end(),
                                  [this](const BallMode& mode) {
                                    ...
                                    return cycleInfo_->getAbsoluteTimeDifference(mode.lastUpdate) >
                                           (mode.measurements < 6
                                                ? std::chrono::duration<float>(mode.measurements) /
                                                      2
                                                : 3s);
                                  }),
                   ballModes_.end());
  ...
  const Rotation2Df r(-odometry.angle());
  ...
  const auto dt =
      std::chrono::duration<float>(cycleInfo_->getAbsoluteTimeDifference(lastPrediction_)).count();
...
void BallFilter::updateValidities()
{
  for (auto& mode : ballModes_)
  {
    ...
    const float maxPerceptsPerSecond = 0.5f / cycleInfo_->cycleTime.count();
