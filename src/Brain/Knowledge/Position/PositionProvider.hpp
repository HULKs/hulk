...
public:
  ...
  /**
   * @brief generate pose on specified quadrant (not to be used during RoboCup Games)
   * @param clusterHint ID of the cluster the sampled position belongs to
   * @param ownHalf whether to generate a pose on the own half
   * @param leftHalf whether to generate a pose on the left half
   * @return a pose for events
   */
  Pose getEventPose(unsigned int& clusterHint, bool ownHalf, bool leftHalf) const;
