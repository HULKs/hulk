...
{
  UKFPose2D::reset(Vector3f{poseMean.x(), poseMean.y(), poseMean.angle()}, poseCov);
                                   ...
                                   const bool pruneByDistanceThreshold) const
{
  ...
  float lineLengthSq = absoluteLineVector.squaredNorm();
  ...
  // a line is considered long if it is significantly longer then the goal box area width. The
  // penalty area width is used since here we have lines that are very close together. Lines longer
  // than the penalty area are always far away from eachother. Thus they can be associated over a
  // longer distance.
  const float significantlyLongerLength = 0.2f + fieldDimensions_.fieldGoalBoxAreaWidth;
  const bool isLongLine = lineLengthSq > significantlyLongerLength * significantlyLongerLength;
  ...
  Vector2f centerOfLineSegment = (absoluteLine.p1 + absoluteLine.p2) / 2.f;
  for (auto& fieldLine : fieldInfo_.lines)
  {
    const float fieldLineLengthSq = (fieldLine.p1 - fieldLine.p2).squaredNorm();
    ...
    if (angle > badAngleThreshold ||
        ...
        (lineLengthSq > fieldLineLengthSq && (isFieldLineAlongAxis(fieldLine) || isLongLine)))
    {
      continue;
    }
                                            ...
                                            const bool ignoreCirclePerceptsNearGoalSupport)
{
  ...
  if (centerCircle.hasOrientation)
  {
    ...
    const Pose observationPose2 =
        Pose(-observationPose1.position(), Angle::normalized(observationPose1.angle() + M_PI));
    ...
    const auto update =
        Angle::angleDiff(observationPose1.angle(), stateMean_.z()) <
                Angle::angleDiff(observationPose2.angle(), stateMean_.z())
            ? Vector3f(observationPose1.x(), observationPose1.y(), observationPose1.angle())
            : Vector3f(observationPose2.x(), observationPose2.y(), observationPose2.angle());
