...
#include "Brain/Knowledge/Position/UKFPositionKnowledge.hpp"
...
UKFPositionKnowledge::UKFPositionKnowledge(const ModuleManagerInterface& manager)
  ...
  , eventMode_(*this, "eventMode", [] {})
  , eventOwnHalf_(*this, "eventOwnHalf", [] {})
  , eventLeftHalf_(*this, "eventLeftHalf", [] {})
  , blindFlight_(*this, "blindFlight", [] {})
  , cycleInfo_(*this)
...
void UKFPositionKnowledge::updateState()
{
  ...
  else
  {
    ...
    if (gameControllerState_->penalty == Penalty::NONE && lastPenalty_ != Penalty::NONE)
    {
      ...
      else
      {
        ...
        {
          preparePoseHypotheses(1, sigmaInitial_(), [this](unsigned int& clusterHint) -> Pose {
            return positionProvider_.getEventPose(clusterHint, eventOwnHalf_(), eventLeftHalf_());
          });
        }
        else
        {
    ...
    else if (gameControllerState_->gameState == GameState::SET)
    {
      if ((!bodyPose_->footContact || wasHighInSet_) &&
          (motionState_->bodyMotion == ActionCommand::Body::MotionType::STAND ||
           motionState_->bodyMotion == ActionCommand::Body::MotionType::PENALIZED))
      {
        wasHighInSet_ = true;
   ...
   * positions)
   */
  const bool sufficientlyStable =
      (motionState_->bodyMotion == ActionCommand::Body::MotionType::WALK ||
       motionState_->bodyMotion == ActionCommand::Body::MotionType::STAND) &&
      !bodyPose_->wonky;
...
bool UKFPositionKnowledge::linesAreValid() const
{
  ...
  return (imageData_->cameraPosition ==
              CameraPosition::TOP || // One can use all images of the top camera
          (imageData_->cameraPosition == CameraPosition::BOTTOM &&
...
Clock::time_point UKFPositionKnowledge::updateLastTimeJumped(const Pose& currentPoseEstimate)
{
  // Calculate the last time jumped
  const float jumpDistThreshSquared = 0.5f * 0.5f;
  const float jumpAngleThresh = 30 * TO_RAD;
  if ((currentPoseEstimate.position() - lastPose_.position()).squaredNorm() >
          jumpDistThreshSquared ||
      Angle::angleDiff(currentPoseEstimate.angle(), lastPose_.angle()) > jumpAngleThresh)
  {
...
void UKFPositionKnowledge::publishPoseEstimate()
{
  ...
  const bool recentIrregularity =
      resettedThisCycle_ || jumpedThisCycle_ ||
  ...
  const bool invalid =
      moreThanOneHypothesis || ((recentIrregularity || isPenalized) && !penaltyShootout);
