...
#include "Brain/Knowledge/Position/UKFPose2D.hpp"
                                ...
                                const Vector3f& odometryProcessNoise)
{
  ...
  mY *= 1.f / 7.f;
  mDirection *= 1.f / 7.f;
  stateMean_ = Vector3f(mX, mY, std::atan2(mDirection.y(), mDirection.x()));
...
void UKFPose2D::poseSensorUpdate(const Vector3f& poseObservation, const Matrix3f& covObservation)
{
  ...
  mY *= 1.f / 7.f;
  mDirection *= 1.f / 7.f;
  predictedObservationMean = Vector3f(mX, mY, std::atan2(mDirection.y(), mDirection.x()));
                                     ...
                                     const Vector3f& referencePose) const
{
  ...
  return Vector3f(c * relativeOdometeryPredict.x() - s * relativeOdometeryPredict.y(),
                  s * relativeOdometeryPredict.x() + c * relativeOdometeryPredict.y(),
                  relativeOdometeryPredict.angle());
}
