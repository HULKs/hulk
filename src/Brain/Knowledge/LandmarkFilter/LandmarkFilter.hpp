...
public:
  ...
  ModuleName name__{"LandmarkFilter"};
  /**
...
private:
  ...
  {
    GoalPost(Vector2f p, Clock::time_point t)
      : position{std::move(p)}
      , timestampLastSeen{t}
    {
    }
    ...
    Clock::time_point timestampLastSeen;
  };
  ...
  const Parameter<Clock::duration> maxGoalPostAge_;
  ...
  Clock::time_point lastLineTimestamp_;
  /// the timestamp of the last used goal data
