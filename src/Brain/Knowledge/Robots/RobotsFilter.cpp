#include "Brain/Knowledge/Robots/RobotsFilter.hpp"
...
{
  if (cycleInfo_->getAbsoluteTimeDifference(robotPosition_->lastTimeJumped) < 0.5s ||
...
{
  trackedRobots_.erase(
      std::remove_if(trackedRobots_.begin(), trackedRobots_.end(),
                     [this](const Robot& robot) {
                       // The more measurements there are for a robot, the longer
                       // it is allowed to stay in the filter.
                       return cycleInfo_->getAbsoluteTimeDifference(robot.lastUpdate) >
                              (robot.measurements < static_cast<unsigned int>(
                                                        timeKeepRobotInFilter_().count() * 2.f)
                                   ? Clock::duration{robot.measurements} / 2.f
...
void RobotsFilter::predictRobots()
{
  ...
  const float deltaT{cycleInfo_->cycleTime.count()};
...
void RobotsFilter::processMeasurements()
{
  ...
  for (const auto& measurement : robotData_->positions)
  {
    ...
    const Matrix2f measurementCov = Rotation2Df(robotPosition_->pose.angle()).inverse() *
                                    projectionMeasurementModel_.computePointCovFromPositionFeature(
                                        measurement, cameraMatrix_->camera2ground) *
                                    Rotation2Df(robotPosition_->pose.angle());
  ...
  for (const auto& measurement : filteredMeasurements)
  {
    if (measurement.associatedRobots.empty())
    {
      ...
      trackedRobots_.emplace_back(
          ...
          initialCov, cycleInfo_->startTime);
      continue;
    }
    for (auto* const associatedRobot : measurement.associatedRobots)
    {
...
void RobotsFilter::publishFilteredRobots()
{
  ...
  {
    const Clock::duration timeSinceLastUpdate{
        cycleInfo_->getAbsoluteTimeDifference(robot.lastUpdate)};
    const float distancePredictedSinceLastUpdate =
        robot.getVelocity().norm() * timeSinceLastUpdate.count();
