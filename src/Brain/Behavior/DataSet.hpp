...
struct DataSet
{
   ...
   * @param spo a reference to the searcher position
   ...
   * @param lp a reference to the loser position
   * @param lrbmt a reference to the last requested body motion type
   */
  DataSet(const ModuleBase& module, const GameControllerState& gcs, const BallState& bs,
          ...
          const SearcherPosition& spo, const FieldDimensions& fd, const StrikerAction& sa,
          const PenaltyStrikerAction& ps, const SetPlayStrikerAction& sps, const KeeperAction& ka,
          const PenaltyKeeperAction& pa, const PointOfInterests& poi, const CycleInfo& ci,
          const SetPosition& sp, const DefenderAction& da, const DefendingPosition& dp,
          const BishopPosition& bp2, const SupportingPosition& sp2,
          const ReplacementKeeperAction& rk, const ButtonData& bd, const WorldState& ws,
          const KickConfigurationData& kcd, const HeadPositionData& hpd, const LoserPosition& lp,
          const ActionCommand::Body::MotionType& lrbmt)
    : parameters(module)
    ...
    , lastRequestedBodyMotionType(lrbmt)
  {
  }
  ...
  /// a reference to the searcher position
  const SearcherPosition& searcherPosition;
  ...
  /// a reference to the loser position
  const LoserPosition& loserPosition;
  ...
  /// a reference to the last body motion type
  const ActionCommand::Body::MotionType& lastRequestedBodyMotionType;
};
