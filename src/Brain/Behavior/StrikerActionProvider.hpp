...
public:
  ...
  ModuleName name__{"StrikerActionProvider"};
  /**
...
private:
  ...
  /// enough)
  const Parameter<float> kickRatingThreshold_;
  ...
  bool wasGivenWayFreeLastCycle_{false};
  KickTarget lastKickTarget_{KickTarget::NONE};
  int lastSign_;
   ...
   * specific foot)
   */
  void createStrikerAction(KickType kickType, const Vector2f& absTarget, int& lastSign,
                           bool forceSign);
   ...
   * specific foot)
   */
  void createStrikerAction(const Vector2f& absTarget, int& lastSign, bool forceSign);
   ...
   */
  void createStrikerAction(InWalkKickType inWalkKickType, const Vector2f& absTarget);
   ...
   */
  float rateKick(const Vector2f& kickTarget, Vector2f leftClipPoint, Vector2f rightClipPoint);
  /**
   ...
   */
  KickTarget checkForBestKickTarget(const Vector2f& left, const Vector2f& center,
                                    const Vector2f& right);
  /**
   * @brief use the collision detector and rateKick to determine a free way in the current
  ...
  /**
   * @brief check whether a side kick is useful. Currently false returning check as this is not
   * implemented
   * @return if the side kick is useful
   */
  bool useSideKick() const;
  /**
   * @brief check whether a turn kick is useful. Currently false returning check as this is not
   * implemented
