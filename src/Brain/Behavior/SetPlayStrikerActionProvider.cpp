...
#include "Brain/Behavior/SetPlayStrikerActionProvider.hpp"
...
SetPlayStrikerActionProvider::SetPlayStrikerActionProvider(const ModuleManagerInterface& manager)
  ...
  , alwaysKickInFreeKick_(*this, "alwaysKickInFreeKick", [] {})
...
void SetPlayStrikerActionProvider::block()
{
  ...
  Vector2f blockerPosition = teamBallModel_->absPosition - 0.85f * ownGoalToBall.normalized();
  ...
  const Vector2f blockerPositionToBall = teamBallModel_->absPosition - blockerPosition;
...
void SetPlayStrikerActionProvider::performFreeKick()
{
  ...
  SetPlayStrikerAction::Type action =
      alwaysKickInFreeKick_() ? SetPlayStrikerAction::Type::KICK : kickOrDribble();
  if (action == SetPlayStrikerAction::Type::KICK)
  {
    createStrikerAction(KickType::FORWARD, ballTarget, teamBallModel_->relPosition, lastSign_,
                        false);
  }
  else
  {
    createStrikerAction(ballTarget, teamBallModel_->relPosition, lastSign_, false);
  }
}
...
Vector2f SetPlayStrikerActionProvider::kickTarget() const
{
  ...
  if (gameControllerState_->setPlay == SetPlay::PUSHING_FREE_KICK)
  {
    return ballInOpponentsCorner ? cornerKickTarget : absOpponentGoal;
  }
  return absOpponentGoal;
...
SetPlayStrikerAction::Type SetPlayStrikerActionProvider::kickOrDribble()
{
  ...
  const float distanceToOpponentGoal = (teamBallModel_->absPosition - absOpponentGoal).norm();
  ballNearOpponentGoal_ =
      Hysteresis::smallerThan(distanceToOpponentGoal, 3.0f, 0.25f, ballNearOpponentGoal_);
  const bool ballInOpponentsCorner = worldState_->ballInCorner && !worldState_->ballInOwnHalf;
  ...
  for (const auto& player : teamPlayers_->players)
  {
    ...
    shouldKick_ = Hysteresis::greaterThan(player.pose.x(), 0.0f, 0.25f, shouldKick_);
