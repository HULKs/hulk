...
#include "Brain/Behavior/BishopPositionProvider.hpp"
...
void BishopPositionProvider::cycle()
{
  ...
  if (allowAggressiveBishop_())
  {
    ...
    if (kickingTeam)
    {
      ...
      {
        // hang around on the same side of the ball during long distance free kicks
        absBishopPosition = Vector2f{absBishopPosition.x(), -1.f * absBishopPosition.y()};
      }
    }

    // move bishop position away from ball if too close
    const Vector2f ballToBishop(absBishopPosition - teamBallModel_->absPosition);
    ...
    const Vector2f bishopToBall = teamBallModel_->absPosition - absBishopPosition;
  ...
  else
  {
    ...
    const Vector2f supportingPositionToBall = teamBallModel_->absPosition - absSupportingPosition;
    ...
    Vector2f absBishopPosition = teamBallModel_->absPosition -
                                 distanceToBall_() * Vector2f(std::cos(angleBishopPositionToBall),
    ...
    const Vector2f bishopPositionToBall = teamBallModel_->absPosition - absBishopPosition;
