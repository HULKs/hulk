...
#include "Brain/Behavior/KeeperActionProvider.hpp"
...
KeeperActionProvider::KeeperActionProvider(const ModuleManagerInterface& manager)
  ...
  , keeperPosition_(Vector2f((-fieldDimensions_->fieldLength * 0.5f) +
                                 fieldDimensions_->fieldGoalBoxAreaLength * 0.5f,
                             0.f))
{
}
...
void KeeperActionProvider::cycle()
{
  ...
  {
    if (strikerIsInOwnGoalBoxArea())
    {
      // if the striker is inside of our own goal box area the keeper position is adjusted to
      // prevent obstructing the striker
      ...
      const Vector2f waitingPosition(keeperPosition_.x(),
                                     teamBallModel_->absPosition.y() + sign * minDistanceToBall);
      const Vector2f waitingPositionToBall = teamBallModel_->absPosition - waitingPosition;
      const float orientation = std::atan2(waitingPositionToBall.y(), waitingPositionToBall.x());
      KeeperAction::Action action(KeeperAction::Type::BLOCK_GOAL,
                                  Pose(waitingPosition, orientation));
      keeperAction_->actions.push_back(action);
    }
    else
    {
      ...
      // the keeper y-position is obtained by projecting the x-position on the goal-center to ball
      // line
      const float interceptYCoord = Range<float>::clipToGivenRange(
          teamBallModel_->absPosition.y() /
              // abs to avoid sign flip when ball is behind the own goal line, epsilon to prevent
              // division by zero
              (std::abs(teamBallModel_->absPosition.x() + fieldDimensions_->fieldLength / 2) +
      ...
      const float interceptAngle =
          std::atan2(teamBallModel_->absPosition.y() - keeperPosition_.y(),
                     teamBallModel_->absPosition.x() - keeperPosition_.x());
      KeeperAction::Action action(KeeperAction::Type::BLOCK_GOAL,
                                  Pose(interceptVec, interceptAngle));
      keeperAction_->actions.push_back(action);
    }
  }
...
bool KeeperActionProvider::shouldSquat() const
{
  ...
  const bool robotLooksForward = Angle::angleDiff(robotPosition_->pose.angle(), 0) <= (100 * TO_RAD);

  return inGoal && goalDirection && std::abs(ballState_->destination.y()) < squatWidth &&
         robotLooksForward;
}

bool KeeperActionProvider::strikerIsInOwnGoalBoxArea() const
{
  for (const auto& teamPlayer : teamPlayers_->players)
  {
    ...
    if (teamPlayer.insideOwnGoalBoxArea)
    {
      return true;
    }
  }
