...
#include "Brain/Behavior/HeadPositionProvider.hpp"
...
HeadPosition HeadPositionProvider::calculateLookAroundBallHeadPositions()
{
  ...
  const float relativeBallAngle =
      std::atan2(teamBallModel_->relPosition.y(), teamBallModel_->relPosition.x());
  const float relativeBallDistanceSquared = teamBallModel_->relPosition.squaredNorm();
...
HeadPosition HeadPositionProvider::calculateLookAroundHeadPositions(float yawMax, float angle)
{
  ...
  innerPositionLeft_.yaw = std::min(angle + (yawMax * .5f), yawMax_());
  outerPositionRight_.yaw = std::max(angle - yawMax, -yawMax_());
  innerPositionRight_.yaw = std::max(angle - (yawMax * .5f), -yawMax_());

  auto timeDiff = cycleInfo_->getAbsoluteTimeDifference(headMotionOutput_->timeWhenReachedTarget);
  switch (nextLookAroundState_)
  {
    case INITIAL:
      lastLookAroundState_ = INITIAL;
      nextLookAroundState_ = GOING_MIDDLE_LEFT;
      currentlyTurningLeft_ = true;
      break;
    case GOING_LEFT:
      ...
      break;
    case GOING_MIDDLE_LEFT:
      lastLookAroundState_ = GOING_MIDDLE_LEFT;
      headPosition = innerPositionLeft_;
      if (std::abs(headMotionOutput_->target[0] - innerPositionLeft_.yaw) <
          targetPositionTolerance_())
      {
        if (timeDiff > timeToRest_())
        {
          if (currentlyTurningLeft_)
          {
    ...
    case GOING_MIDDLE:
      ...
      if (std::abs(headMotionOutput_->target[0] - innerPosition_.yaw) < targetPositionTolerance_())
      {
        if (timeDiff > timeToRest_())
        {
          nextLookAroundState_ = GOING_MIDDLE_RIGHT;
          if (currentlyTurningLeft_)
          {
            nextLookAroundState_ = GOING_MIDDLE_LEFT;
          }
      ...
      break;
    case GOING_MIDDLE_RIGHT:
      lastLookAroundState_ = GOING_MIDDLE_RIGHT;
      headPosition = innerPositionRight_;
      if (std::abs(headMotionOutput_->target[0] - innerPositionRight_.yaw) <
          targetPositionTolerance_())
      {
        if (timeDiff > timeToRest_())
        {
