...
#include "Brain/Behavior/PenaltyKeeperActionProvider.hpp"
...
PenaltyKeeperActionProvider::PenaltyKeeperActionProvider(const ModuleManagerInterface& manager)
  ...
  , goalLineHalfWithTolerance_(fieldDimensions_->fieldGoalBoxAreaWidth / 2.0 +
...
void PenaltyKeeperActionProvider::cycle()
{
  ...
  if (gameControllerState_->gamePhase != GamePhase::PENALTYSHOOT &&
      gameControllerState_->setPlay != SetPlay::PENALTY_KICK && !gameControllerState_->kickingTeam)
  {
    return;
  }
  ...
  { /// No ballstate confidence check. We are reducing reliability.
    ...
    /// (x) and will be in fieldGoalBoxAreaWidth/2 (y)
