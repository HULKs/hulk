...
private:
  ...
  /// valid. Unit: Seconds
  const Parameter<Clock::duration> maxWhistleTimeDiff_;
  /// duration to wait for a gamecontroller message to acknowledge transition to READY
  const Parameter<Clock::duration> maxWaitForReadyMessage_;
  ...
  Clock::time_point stateChanged_;
  /// time when the nao started listening for a whistle
  Clock::time_point lastTimeStartedWhistleDetection_;
};
