...
#include "PenaltySpotDetection.hpp"
...
void PenaltySpotDetection::detectPenaltySpot()
{
  ...
  for (const auto& hSegment : filteredSegments_->horizontal)
  {
    ...
    if (!cameraMatrix_->getPixelRadius(imageData_->image422.size, seed,
                                       fieldDimensions_->fieldPenaltyMarkerSize / 2,
                                       expectedRadius))
    {
      continue;
    }
    ...
    // 444
    assert(expectedRadius > 0);
    float ratio = hSegmentPixelLength / (float)expectedRadius;
    ...
    for (const auto& vSegment : filteredSegments_->vertical)
    {
      ...
      cameraMatrix_->pixelToRobot(vSegment->start, rStart);
      cameraMatrix_->pixelToRobot(vSegment->end, rEnd);
      assert(vSegment->start.y() <= vSegment->end.y());
      float distance = (rEnd - rStart).norm();
      ...
      if (excludeBall_())
      {
        ...
        // Check whether the seed is on the filtered projected ball
        if (seed.x() >= ballData_->filteredProjectedBall.center.x() -
                            ballData_->filteredProjectedBall.radius &&
            seed.y() >= ballData_->filteredProjectedBall.center.y() -
                            ballData_->filteredProjectedBall.radius &&
            seed.x() <= ballData_->filteredProjectedBall.center.x() +
                            ballData_->filteredProjectedBall.radius &&
            seed.y() <= ballData_->filteredProjectedBall.center.y() +
                            ballData_->filteredProjectedBall.radius)
        {
      ...
      int seedY = static_cast<int>(seedColor.y1_);
      int seedChroma = std::abs(static_cast<int>(seedColor.cb_) - 128) +
                       std::abs(static_cast<int>(seedColor.cr_) - 128);
      ...
      Vector2f radiusVector = Vector2f(expectedRadius, vSegmentPixelLength / 2);
      ...
      for (const auto& d : directions)
      {
        ...
        const int pointY = static_cast<int>(std::max(pointColor.y1_, pointColor.y2_));
        const int pointChroma = std::abs(static_cast<int>(pointColor.cb_) - 128) +
                                std::abs(static_cast<int>(pointColor.cr_) - 128);
      ...
      penaltySpotSeeds_.back().expectedRadius = expectedRadius;
  ...
  if (cameraMatrix_->pixelToRobot(penaltySpotSeeds_[0].pixelPosition, robotCoordinates))
  {
    penaltySpotSeeds_[0].relativePosition = robotCoordinates;
    penaltySpotData_->penaltySpot = penaltySpotSeeds_[0];
    penaltySpotData_->valid = true;
    penaltySpotData_->timestamp = imageData_->timestamp;
  }
}
...
void PenaltySpotDetection::sendImagesForDebug()
{
  ...
  if (debug().isSubscribed(mount))
  {
    ...
    for (const auto& spotSeed : penaltySpotSeeds_)
    {
      image.ellipse(
          ...
          Color::ORANGE, 100);
      image.circle(Image422::get444From422Vector(spotSeed.pixelPosition), 2, Color::ORANGE);
      image.line(Image422::get444From422Vector(spotSeed.hSegment->start),
                 Image422::get444From422Vector(spotSeed.hSegment->end), Color::ORANGE);
      image.line(Image422::get444From422Vector(spotSeed.vSegment->start),
    ...
    if (penaltySpotData_->valid)
    {
      image.ellipse(
          ...
          0, Color::RED, 100);
      image.circle(Image422::get444From422Vector(penaltySpotData_->penaltySpot.pixelPosition), 2,
                   Color::RED);
      image.line(Image422::get444From422Vector(penaltySpotData_->penaltySpot.hSegment->start),
                 Image422::get444From422Vector(penaltySpotData_->penaltySpot.hSegment->end),
                 Color::RED);
      image.line(Image422::get444From422Vector(penaltySpotData_->penaltySpot.vSegment->start),
                 Image422::get444From422Vector(penaltySpotData_->penaltySpot.vSegment->end),
                 Color::RED);
      image.drawString(std::to_string((int)penaltySpotData_->penaltySpot.score),
                       Image422::get444From422Vector(penaltySpotData_->penaltySpot.pixelPosition),
                       Color::BLACK);
      for (const auto& point : penaltySpotData_->penaltySpot.debugPoints)
      {
        image.circle(Image422::get444From422Vector(point), 3, Color::RED);
      }
    }
  ...
  if (debug().isSubscribed(mount))
  {
    ...
    for (int y = 0; y < chromaImage.size_.y(); ++y)
    {
      for (int x = 0; x < chromaImage.size_.x(); ++x)
      {
        int cb = imageData_->image422.at(y, x / 2).cb_;
        int cr = imageData_->image422.at(y, x / 2).cr_;
        int sat = std::abs(cb - 128) + std::abs(cr - 128);
        chromaImage.at(y, x).y_ = sat;
        chromaImage.at(y, x).cb_ = 128;
        chromaImage.at(y, x).cr_ = 128;
      }
    }
