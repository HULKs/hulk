...
SensorFusion::SensorFusion(const ModuleBase& module)
  ...
  , gyro_bias_alpha_(module, "gyro_bias_alpha",
                     [] {}) /// TODO: Maybe do some sort of auto calibration
...
{
  double accNorm = extAccel.norm();
  if (std::fabs(accNorm - gravity_()) > acceleration_threshold_())
  {
    return false;
  }

  Vector3d gyroDelta = (extGyro - gyro_prev_).cwiseAbs();
  if (gyroDelta(0) > delta_angular_velocity_threshold_() ||
      gyroDelta(1) > delta_angular_velocity_threshold_() ||
      gyroDelta(2) > delta_angular_velocity_threshold_())
  {
    return false;
  }

  Vector3d gyro = (extGyro - gyro_bias_).cwiseAbs();
  return !(gyro(0) > angular_velocity_threshold_() || gyro(1) > angular_velocity_threshold_() ||
           gyro(2) > angular_velocity_threshold_());
}

void SensorFusion::updateGyroBias(const Vector3d& extGyro, const Vector3d& extAccel)
{
  bool steadyState = checkSteadyState(extGyro, extAccel);
...
void SensorFusion::setOrientation(const Vector3d& orient)
{
  ...
  const auto& roll = orient(0);
  const auto& pitch = orient(1);
  const auto& yaw = orient(2);
...
Matrix3f SensorFusion::getBodyTilt() const
{
  ...
  //
  // (cos(rpy.z)*cos(rpy.y), cos(rpy.z)*sin(rpy.y)*sin(rpy.x)-sin(rpy.z)*cos(rpy.x),
  // cos(rpy.z)*sin(rpy.y)*cos(rpy.x)+sin(rpy.z)*sin(rpy.x),
  //  sin(rpy.z)*cos(rpy.y), sin(rpy.z)*sin(rpy.y)*sin(rpy.x)+cos(rpy.z)*cos(rpy.x),
  //  sin(rpy.z)*sin(rpy.y)*cos(rpy.x)-cos(rpy.z)*sin(rpy.x), -sin(rpy.y), cos(rpy.y)*sin(rpy.x),
  //  cos(rpy.y)*cos(rpy.x));
  //
  // By canceling some ones and zeros:
  const float sx = std::sin(rpy.x());
  const float cx = std::cos(rpy.x());
  const float sy = std::sin(rpy.y());
  const float cy = std::cos(rpy.y());
...
{
  auto q = global_to_local_.inverse();
  double theta = 2 * std::acos(q.x());

  Vector3f w{Vector3f::Zero()};
  if (theta != 0)
  {
    double s = 1 / (std::sin(theta / 2));
