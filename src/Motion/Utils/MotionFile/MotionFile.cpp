...
bool MotionFile::loadFromFile(const std::string& filename)
{
  ...
  if (!f.is_open())
  {
    Log<M_MOTION>(LogLevel::ERROR) << "MotionFile " << filename << " could not be opened";
    return false;
  }
  try
  {
  ...
  {
    Log<M_MOTION>(LogLevel::ERROR) << "Could not parse MotionFile " << filename;
    return false;
  }
...
bool MotionFile::verify() const
{
  if (header.time < 0)
  {
    Log<M_MOTION>(LogLevel::ERROR) << "MotionFile " << header.title << ": header time is < 0";
    return false;
  }
  if (header.joints.empty() || header.joints.size() > static_cast<std::size_t>(Joints::MAX))
  {
    Log<M_MOTION>(LogLevel::ERROR)
        << "MotionFile " << header.title << ": invalid number of joints specified in header";
    return false;
  }
  for (auto joint : header.joints)
  {
    if (joint < 0 || static_cast<std::size_t>(joint) >= static_cast<std::size_t>(Joints::MAX))
    {
      Log<M_MOTION>(LogLevel::ERROR)
          << "MotionFile " << header.title << ": invalid joint specified in header";
      return false;
    }
  }
  if (header.version != "2.0")
  {
    Log<M_MOTION>(LogLevel::ERROR) << "MotionFile " << header.title << ": version not 2.0";
    return false;
  }
  for (const auto& pos : position)
  {
    if (pos.time < 0)
    {
      Log<M_MOTION>(LogLevel::ERROR) << "MotionFile " << header.title << ": position time is < 0";
      return false;
    }
  ...
  for (const auto& stiff : stiffness)
  {
    if (stiff.time < 0)
    {
      Log<M_MOTION>(LogLevel::ERROR) << "MotionFile " << header.title << ": stiffness time is < 0";
      return false;
    }
