...
public:
  ...
  ModuleName name__{"HeadMotion"};
  /**
   * @brief HeadMotion initializes members
   * @param a reference to motion
   */
  explicit HeadMotion(const ModuleManagerInterface& manager);
  /**
...
private:
   ...
   */
  JointsHeadArray<float> calculateHeadAnglesFromTarget(const Vector3f& targetPosition,
  ...
  /// whether to prevent negative head angles
  const Parameter<bool> limitHeadPitch_;
  ...
  Clock::time_point timeWhenReachedTarget_;
  /// the head joint angles that are currently active
  JointsHeadArray<float> jointAngles_{};
};
