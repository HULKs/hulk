...
class UnixSocketConfig::Impl :
    public std::enable_shared_from_this<UnixSocketConfig::Impl>
{
private:
...
void UnixSocketConfig::Impl::startAccept()
{
  ...
  print( "UnixSocketConfig: Waiting for connection", LogLevel::INFO );
...
{
  print( "UnixSocketConfig: receiving header", LogLevel::INFO );
...
void UnixSocketConfig::Impl::onReceiveHeader(const boost::system::error_code& error)
{
  ...
  if ( error )
  {
    print( "UnixSocketConfig: error while receiving header", LogLevel::WARNING );
    return;
  }

  print( "UnixSocketConfig: received header", LogLevel::INFO );
...
void UnixSocketConfig::Impl::onReceiveBody(const boost::system::error_code& error)
{
  if ( error )
  {
    print( "UnixSocketConfig: error while receiving body", LogLevel::WARNING );
    return;
  }
  ...
  {
    print( "UnixSocketConfig: received message type CM_SET: "+body, LogLevel::INFO );
    ...
    for( auto it = root.begin(); it != root.end(); ++it ) {
      ...
      try {
        ...
        print(std::string("UnixSocketConfig: Exception from Configuration: ") + e.what(), LogLevel::ERROR);
      }
    }
  }
  if ( header_.msgType == CM_GET_MOUNTS )
  {
    print( "UnixSocketConfig: received message type CM_GET_MOUNTS: "+body, LogLevel::INFO );
    transmitMountList();
  }
  if ( header_.msgType == CM_GET_KEYS )
  {
    print( "UnixSocketConfig: received message type CM_GET_KEYS: " + body, LogLevel::INFO);
    transmitKeyList(body);
  }
  if ( header_.msgType == CM_SAVE )
  {
    print( "UnixSocketConfig: received message type CM_SAVE: "+body, LogLevel::INFO );
    try {
      config_.save();
    } catch (const ConfigurationException& e) {
      print(std::string("UnixSocketConfig: Exception from Configuration: ") + e.what(), LogLevel::ERROR);
