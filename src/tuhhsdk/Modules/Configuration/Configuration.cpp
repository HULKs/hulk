...
{
  bool found = false;
  std::string headBodyDefaultPath = ((type == ConfigurationType::HEAD) ? "head/" : "body/") + std::string("default/");
  std::string headBodyPath = ((type == ConfigurationType::HEAD) ? ("head/" + naoHeadName_) : ("body/" + naoBodyName_)) + "/";
  // Try the most generic configuration first and the most specific configuration last since it will overwrite the previous values.
  ...
  if (!found) {
    throw ConfigurationException("Configuration file '" + name + "' does not exist in any configuration directory.", ConfigurationException::FILE_NOT_FOUND);
  }
}
...
bool Configuration::mountFile(const std::string &mount, const std::string &filename)
{
  ...
  if (from.root.type() != Uni::ValueType::OBJECT){
    throw ConfigurationException("Configuration files must contain a Json::objectValue as root node!", ConfigurationException::INVALID_JSON_FILE);
  }
  ...
  Log(LogLevel::INFO) << "mounted " << filename << " to " << mount;
  return true;
}
...
bool Configuration::hasProperty(const std::string& mount, const std::string& key)
{
  if(mountPts_.count(mount) > 0){
    ...
    throw ConfigurationException("Mount Point " + mount + " does not exist!", ConfigurationException::MOUNT_POINT_NOT_EXISTING);
  }
}
...
Uni::Value& Configuration::get(const std::string& mount, const std::string& key){
  if(mountPts_.count(mount) > 0){
    if (!mountPts_[mount].root.hasProperty(key)) {
      throw ConfigurationException("Key " + key + " does not exist in mount point " + mount + "!", ConfigurationException::KEY_NOT_EXISTING);
    }
...
{
  if (mountPts_.find(mount) != mountPts_.end()) {
    // key denotes the key that was received, realKey is the top-level part of the key as it is used in the map.
    std::string realKey;
    ...
    while (currentPos < key.length()) {
      ...
      std::string thisKey = key.substr(currentPos, (nextPos == std::string::npos ? nextPos : nextPos - currentPos));
      ...
      // After accessing the new value with [] or at, the type will be fixed to OBJECT or ARRAY (see Uni::Value).
      ...
      if (bracketPos < dotPos) {
        ...
        if (closingBracketPos == std::string::npos) {
          throw ConfigurationException("Key has no matching closing bracket.", ConfigurationException::INVALID_KEY);
        }
        std::string indexString = key.substr(bracketPos + 1, closingBracketPos - bracketPos - 1);
        // TODO: check that everything is valid between the brackets. But this is something we might not care.
        ...
        if (index >= currentValue->size()) {
          ...
          throw ConfigurationException("Enlarging arrays via Configuration::set is not supported yet.", ConfigurationException::INVALID_KEY);
        }
    ...
    if (currentValue == nullptr || completeValue == nullptr) {
      throw std::runtime_error("Something went wrong in Configuration::set. This is probably a bug in the code.");
    }
    *currentValue = value;
    ...
    throw ConfigurationException("Mount Point does not exist!", ConfigurationException::MOUNT_POINT_NOT_EXISTING);
  }
}
...
void Configuration::save(){
  try{
    ...
    throw ConfigurationException("Can not save Configuration file to disk. " + std::string(e.what()), ConfigurationException::ERROR_WHILE_SAVING);
  }
}
...
boost::signals2::connection Configuration::registerCallback(const std::string &mount, const std::string &key, ConfigurationCallback callback)
{
