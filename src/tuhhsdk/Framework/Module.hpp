...
private:
  ...
  template<typename T, typename T2> friend class Module;
  template<typename T> friend class Dependency;
  template<typename T> friend class Reference;
  template<typename T> friend class Production;
  template<typename T> friend class Parameter;
};
...
public:
  ...
  /// the first element of the linked list (the last constructed module). It must be defined in e.g. the module manager file.
...
public:
   ...
   */
  Module(const ModuleManagerInterface& manager, const std::string& name) :
    ModuleBase(manager, name)
  {
...
private:
  ...
  static ModuleFactory<T, T2> factory_;
};

template<typename T, typename T2>
ModuleFactory<T, T2> Module<T, T2>::factory_;

template<typename T>
class Dependency {
public:
  /**
   * @brief Dependency obtains a reference to the datum from the database and registers in the module
...
public:
  /**
   * @brief Production obtains a reference to the datum from the database and registers in the module
   * @param module the module owning this Production
   */
  Production(ModuleBase& module) :
    data_(module.database_.get<T>())
  {
  ...
  /**
   * @brief operator-> a const version because the producing function may have const methods accessing data
   * @return a pointer to the datum in the database
   */
  const T* operator->() const
  {
...
public:
   ...
   */
  Parameter(const ModuleBase& module, const std::string& key, std::function<void()> callback = std::function<void()>()) :
    value_(),
    callback_(callback)
  {
    module.configuration_.get(module.mount_, key) >> value_;
    if (callback_) {
      module.configuration_.registerCallback(module.mount_, key, boost::bind(&Parameter<T>::onUpdate, this, _1));
    }
  }
  /**
...
private:
  /**
   * @brief onUpdate is called by the Configuration class whenever the value changes (e.g. over network)
...
template<typename T> ModuleFactoryBase<T>* ModuleFactoryBase<T>::begin = nullptr;
