...
#include "Vision/FieldColorDetection/OneMeansFieldColorDetection.hpp"
  ...
  , updateInitialGuessBottom_(false)
{
  fieldColor_->isFieldColor = [this](const YCbCr422& pixel) -> float {
    const int cb = (pixel.cb - meanCb_);
    const int cr = (pixel.cr - meanCr_);
    return (pixel.y1 < thresholdY_ && pixel.y2 < thresholdY_ &&
                                           ...
                                           const int startY)
{
  ...
  for (int y = startY; y < image.size.y(); y += sampleRate_)
  {
    for (int x = 0; x < image.size.x(); x += sampleRate_ / 2)
    {
      ...
      if (pixel.y1 < initCluster.yThresh)
      {
        const Vector2f pixelColor = Vector2f(pixel.cb, pixel.cr);
        const Vector2f colorErr = initCluster.mean - pixelColor;
        const int dist =
            static_cast<int>(colorErr.x() * colorErr.x() + colorErr.y() * colorErr.y() * 2);
        if (dist < maxDist)
        {
          mean += pixelColor;
          meanY += pixel.y1;
...
void OMFCD::sendImageForDebug(const Image422& image)
{
  ...
  { // This only sends every third image because the
    ...
    for (int y = horizonY_; y < fieldColorImage.size.y(); y += 2)
    {
    ...
    Vector2i p2(fieldColorImage.size.x() - 1, horizonY_);
