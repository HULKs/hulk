...
public:
  ...
  ModuleName name__{"ChromaticityFieldColorDetection"};
...
private:
  ...
  const Parameter<std::array<float, 2>> redChromaticityThreshold_;
  /// field color must have greater green chromaticity as this (0.5 sure)
  const Parameter<std::array<float, 2>> lowerGreenChromaticityThreshold_;
  /// field color must have greater green chromaticity as this to be 100 percent sure field color
  const Parameter<std::array<float, 2>> upperGreenChromaticityThreshold_;
  /// field color must have less blue chromaticity as blueChromaticityThreshold_
  const Parameter<std::array<float, 2>> blueChromaticityThreshold_;

  /// produces the isFieldColor function of FieldColor DataType
  void setIsFieldColorFunctionForTop();
  void setIsFieldColorFunctionForBottom();
