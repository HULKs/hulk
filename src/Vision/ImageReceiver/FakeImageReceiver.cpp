#include "Vision/ImageReceiver/FakeImageReceiver.hpp"
...
{
  topCamera2headUncalib_ = KinematicMatrix::transZ(63.64) * KinematicMatrix::transX(58.71) *
                           KinematicMatrix::rotY(0.0209);
  bottomCamera2headUncalib_ = KinematicMatrix::transZ(17.74) * KinematicMatrix::transX(50.71) *
...
void FakeImageReceiver::cycle()
{
  if (!imageData_->valid)
  {
    ...
    // call produceImageData for the purpose thread of synchronization
    robotInterface().produceImageData(*cycleInfo_, dummyImageData);
    fakeImageData_->imageSize =
        dummyImageData.cameraPosition == CameraPosition::TOP ? topImageSize_ : bottomImageSize_;
    ...
    const auto& torso2ground = robotKinematics_->torso2ground;
    if (dummyImageData.cameraPosition == CameraPosition::TOP)
    {
      fakeCameraMatrix_->camera2ground = torso2ground * head2torso * topCamera2headUncalib_;
      fakeCameraMatrix_->camera2torso = head2torso * topCamera2headUncalib_;
      fakeCameraMatrix_->fc = topFc_;
      fakeCameraMatrix_->cc = topCc_;
    }
    else
    {
      fakeCameraMatrix_->camera2ground = torso2ground * head2torso * bottomCamera2headUncalib_;
      fakeCameraMatrix_->camera2torso = head2torso * bottomCamera2headUncalib_;
      fakeCameraMatrix_->fc = bottomFc_;
      fakeCameraMatrix_->cc = bottomCc_;
    }
    // fc and cc have to be scaled for the image resolution
    fakeCameraMatrix_->fc.x() *= static_cast<float>(fakeImageData_->imageSize.x());
    fakeCameraMatrix_->fc.y() *= static_cast<float>(fakeImageData_->imageSize.y());
    fakeCameraMatrix_->cc.x() *= static_cast<float>(fakeImageData_->imageSize.x());
    fakeCameraMatrix_->cc.y() *= static_cast<float>(fakeImageData_->imageSize.y());
    ...
    fakeCameraMatrix_->camera2torsoInv = fakeCameraMatrix_->camera2torso.inverted();
    fakeCameraMatrix_->camera2groundInv = fakeCameraMatrix_->camera2ground.inverted();
