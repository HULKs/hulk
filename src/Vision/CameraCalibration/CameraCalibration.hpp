...
public:
  ...
  ModuleName name__{"CameraCalibration"};
  /**
  ...
  /**
   * @brief cycle draws an image of some defined points to see how to adjust the camera calibration
   * parameters
   */
  void cycle();
...
private:
   ...
   * The feet should be perfectly parallel and and the middle point of the field should be exactly
   * under his torso.
   *
   * @author Thomas Schattschneider
   */
  void projectPenaltyAreaOnImages();
  /// rotates the reference points in the debug image by 90 degrees (use yaw offset with a rotated
  /// robot)
  const Parameter<bool> rotate90Degrees_;
