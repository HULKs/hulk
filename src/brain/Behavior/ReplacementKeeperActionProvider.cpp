...
#include "ReplacementKeeperActionProvider.hpp"
...
void ReplacementKeeperActionProvider::considerSetPlay()
{
  ...
  const Vector2f ballToTarget =
      replacementKeeperAction_->action.pose.position - teamBallModel_->position;
  ...
  if (ballToRobot.norm() < 0.9f || ballToTarget.norm() < 0.9f)
  {
    ...
    replacementKeeperAction_->action.pose.position.y() = newYCoord;
  }
}
