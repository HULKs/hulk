...
PenaltyKeeperActionProvider::PenaltyKeeperActionProvider(const ModuleManagerInterface& manager)
  ...
  , genuflectThreshold_(*this, "genuflectThreshold", [] {})
  ...
  , goalLineHalfWithTolerance_(fieldDimensions_->fieldPenaltyAreaWidth / 2.0 + goalLineLenTolerance_())
{
}
...
void PenaltyKeeperActionProvider::cycle()
{
  ...
  if (gameControllerState_->gamePhase != GamePhase::PENALTYSHOOT && !gameControllerState_->kickingTeam)
  {
    return;
  }

  /// if the game state is not playing or penalized, reset to wait.
  if (gameControllerState_->gameState != GameState::PLAYING || gameControllerState_->penalty != Penalty::NONE)
  {
  ...
  { /// No ballstate confidence check. We are reducing reliability.
    /// if ball destination will be at least minBallDestinationToRobotThresh_ or lesser from robot (x) and will be in fieldPenaltyAreaWidth/2 (y)
    if (ballState_->destination.x() < minBallDestinationToRobotThresh_())
    {
      ...
      ballPosToDestDiff = ballPosToDestDiff.normalized() * (ballPosToDestDiff.norm() + ballDestinationTolerance_());

      float goalLineDest = -ballPosToDestDiff.y() * ballState_->position.x() / ballPosToDestDiff.x();
      if (goalLineDest < goalLineHalfWithTolerance_)
      {
        if (std::abs(goalLineDest) < genuflectThreshold_())
        { // genuflect
          penaltyAction_->type = PenaltyKeeperAction::Type::GENUFLECT;
        }
        else
        {
          if (goalLineDest > 0)
