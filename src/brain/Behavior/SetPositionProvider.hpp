...
private:
  /**
   * @brief getPermutationValue computes a value for a given position assignment (the less the better)
   ...
   * @param remainingTeamPlayers the positions of the team members which are neither keeper nor kickoff striker
   * @param signY the sign by which the y coordinates of the positions are multiplied (for mirroring on the x-axis)
   */
  float getPermutationValue(const std::vector<unsigned int>& perm, const VecVector2f& positions, const VecVector2f& remainingTeamPlayers,
                            const float signY) const;
  /**
   * @brief roleIsCompatibleWithPosition checks whether a certain role may occupy a certain set position
   ...
   */
  bool roleIsCompatibleWithPosition(const PlayingRole role, const unsigned int posIndex) const;
