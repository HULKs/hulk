...
public:
   ...
   */
  PositionProvider(const ModuleBase& module, const FieldInfo& fieldInfo, const GameControllerState& gameControllerState,
   ...
   */
  Pose getManuallyPlaced(unsigned int& clusterHint) const;
  /**
...
private:
  ...
  /// instead of using a single hypothesis in INITIAL, a distribution over the whole sidelines in the own half is generated
  ...
  /// a counter for switching between multiple hypotheses (e.g. when penalized or manually placed). It does not matter whether it overflows.
