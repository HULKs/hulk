...
/**
 * @brief The PositionKnowledge class
 */
class PositionKnowledge : public Module<PositionKnowledge, Brain>, public Uni::To
{
public:
  /**
   * @brief PositionKnowledge initializes the members of the PositionKnowledge
   * @param manager a reference to the brain
   */
  PositionKnowledge(const ModuleManagerInterface& manager);
  /**
...
private:
  ...
  /**
   * @brief resample will modify the sample set s.t. the number of particles at a certain position is proportional to its weight
   */
  void resample();
  /**
  ...
  /**
   * @brief updateWithLine: Each particle is updated with a line by placing the recognized line in respect of the particles position
   ...
   */
  void updateWithLine(PositionParticle& particle, const Line<float>& line, const float projectionDistance);
  /**
   * @brief updateWithLandMarkPosition updates a given particle with relative landmark measurement (e.g. the position of the center circle)
   ...
   * @param absoluteGroundTruthPosition the point on th map (ground truth), where this feature should be
   */
  void updateWithLandMarkPosition(PositionParticle& particle, const Vector2f& relativeLandmarkMeasurement, const float measurementVariance,
                                  const Vector2f& absoluteGroundTruthPosition);
  /**
   * @brief updateWithGoalPosts: Each particle is updated with the goal posts by placing the recognized goal post in respect of the particles position
  ...
  /**
   * @brief angleBetweenLineVectors calculates the smallest angle between two lines (range from 0 to M_PI_2)
  ...
  /**
   * @brief http://stackoverflow.com/a/1501725/2169988 (find the shortest distance between a point and a line segment)
  ...
  /**
   * @brief weightByLine calculates the weight of the particle for a percepted line (in field coordinates, lower weight when percepted line matches no line on
   * field)
   ...
   * @return a weight that corresponds to the probability that this particle a good position estimation
   */
  ...
  const Parameter<float> maxProjectionDistance_;
  /// the maximum allowed projection distance of a line in cases with lower measurement noise
  const Parameter<float> maxProjectionDistanceLowNoise_;
  ...
  /// whether to transmit all particles with seen lines etc. (makes walking with the robot impossible)
  const Parameter<bool> transmitParticles_;
  /// some details about the cycle time
  const Dependency<CycleInfo> cycleInfo_;
  /// line result from vision
  ...
  /// the current max projection distance
  float currentMaxProjectionDistance_;
