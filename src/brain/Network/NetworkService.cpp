...
void NetworkService::cycle()
{
  ...
  if (getTimeDiff(lastTimeQueried_, cycleInfo_->startTime, TDT::SECS) < 1.f)
  {
...

  {
    ...
    {
      Log(LogLevel::INFO) << "WIFI interface changed state to "
                          << (isConnectedToAnyWifi_ ? "CONNECTED" : "DISCONNECTED");
    }

    if (isConnectedToAnyEth_ != wasConnectedToAnyEth)
    {
      Log(LogLevel::INFO) << "Ethernet interface changed state to "
...
bool NetworkService::updateInterfaces()
{
  ...
  {
    Log(LogLevel::ERROR) << "Unable to get network interface information. Reason: "
                         << std::strerror(errno);
    return false;
  }
