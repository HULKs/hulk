...
pub fn generate_cyclers(cyclers: &Cyclers) -> Result<TokenStream, Error> {
    let cyclers = cyclers
        .cyclers
        .iter()
...
fn generate_module(cycler: &Cycler, cyclers: &Cyclers) -> Result<TokenStream, Error> {
...
fn generate_node_fields(cycler: &Cycler) -> TokenStream {
    let fields: Vec<_> = cycler
        ...
        .map(|node| {
            ...
            quote! {
                #node_name_snake_case: crate::#cycler_module_name::#node_module::#node_name
            }
        })
        .collect();
    quote! {
...
fn generate_node_initializers(cycler: &Cycler) -> TokenStream {
    let initializers = cycler.nodes.iter().map(|node| {
        ...
        quote! {
            let #node_name_snake_case = crate::#cycler_module_name::#node_module::#node_name::new(
                crate::#cycler_module_name::#node_module::CreationContext {
                    #field_initializers
                }
            )
...
fn generate_node_field_initializers(node: &Node, cycler: &Cycler) -> TokenStream {
    node.contexts
        ...
        .map(|field| match field {
            ...
            Field::CyclerInstance { name } => quote! {
                #name: instance,
            },
            ...
            if #are_required_inputs_some {
                let main_outputs = {
                    ...
                    self.#node_member.cycle(
                        crate::#cycler_module_name::#node_module::CycleContext {
