...
impl OrientationFilter {
    pub fn update(
        ...
        if self.is_in_steady_state(
            ...
            self.orientation = self.predict_orientation(measured_angular_velocity, cycle_time);
            if let Some(correction) =
                self.correct_orientation(measured_acceleration, parameters.acceleration_weight)
            {
    ...
    fn predict_orientation(
        ...
        let angular_velocity = measured_angular_velocity - self.angular_velocity_bias;
        ...
        // Equation 38
        let angular_rate_derivative = -0.5 * (angular_rate * self.orientation.quaternion());
        // Equation 42
        UnitQuaternion::from_quaternion(
            self.orientation.quaternion() + angular_rate_derivative * cycle_time,
        )
    }
    ...
    fn is_in_steady_state(
        ...
        let angular_velocity_delta =
            (measured_angular_velocity - self.previous_angular_velocity).abs();
...
mod test {
    ...
    fn half_rotation() {
        ...
        let mut orientation_filter = OrientationFilter::default();
        ...
        assert_relative_eq!(orientation_filter.yaw().angle(), PI, epsilon = 1e-3);
    }
    ...
    fn half_rotation_with_noise() {
        ...
        assert_relative_eq!(orientation_filter.yaw().angle(), -PI, epsilon = 1e-2);
    }
    ...
    fn full_rotation() {
        ...
        assert_relative_eq!(orientation_filter.yaw().angle(), 0.0, epsilon = 1e-2);
    }
    ...
    fn full_rotation_with_noise() {
        ...
        assert_relative_eq!(orientation_filter.yaw().angle(), 0.0, epsilon = 1e-1);
    }
