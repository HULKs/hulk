...
impl Cyclers {
    pub fn try_from_directory(root: impl AsRef<Path>) -> Result<Self, Error> {
        let path_to_configuration = root.as_ref().join("framework.toml");
        let configuration: FrameworkConfiguration = toml::from_str(
            &read_to_string(&path_to_configuration).map_err(|error| Error::Io {
                source: error,
                path: path_to_configuration.clone(),
            })?,
        )
    ...
    pub fn try_from_configurations<'config>(
        ...
        let cyclers = values
            .into_iter()
            .map(move |configuration| Self::try_from_configuration(configuration, root))
            .collect::<Result<_, _>>()?;
        Ok(Self { cyclers })
    }
