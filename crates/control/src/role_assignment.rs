...
impl RoleAssignment {
    ...
    fn role_from_state_machine(
        ...
        for event in events {
            ...
            match (new_role, self.loser_since) {
                (Role::Loser, None) => self.loser_since = Some(SystemTime::now()),
                (Role::Loser, Some(..)) => {}
                _ => self.loser_since = None,
            }
        }
...
mod test {
    ...
    proptest! {
        ...
        fn process_role_state_machine_should_be_idempotent_with_event_none(
            ...
            loser_since in Just(Some(Duration::from_secs(4))),
