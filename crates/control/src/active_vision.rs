...
pub struct ActiveVision {
    field_mark_positions: Vec<Framed<Field, Point2<f32>>>,
...
pub struct MainOutputs {
    pub position_of_interest: MainOutput<Framed<Ground, Point2<f32>>>,
}
...
fn is_position_visible(
    ...
    UnitComplex::rotation_between(&Vector2::x(), &position.inner.coords)
        .angle()
        .abs()
        < parameters.angle_threshold
        && position.inner.coords.norm() < parameters.distance_threshold
}

fn closest_field_mark_visible(
    field_mark_positions: &[Framed<Field, Point2<f32>>],
    parameters: &LookActionParameters,
    ground_to_field: &Transform<Ground, Field, Isometry2<f32>>,
) -> Option<Framed<Ground, Point2<f32>>> {
    field_mark_positions
        .iter()
        .map(|position| ground_to_field.inverse() * position)
        .filter(|position| is_position_visible(*position, parameters))
        .min_by_key(|position| NotNan::new(position.inner.coords.norm()).unwrap())
}
...
fn generate_field_mark_positions(
    field_dimensions: &FieldDimensions,
) -> Vec<Framed<Field, Point2<f32>>> {
    let left_center_circle_junction =
        point![0.0, field_dimensions.center_circle_diameter / 2.0].framed();
    let right_center_circle_junction =
        point![0.0, -field_dimensions.center_circle_diameter / 2.0].framed();
    let left_center_t_junction = point![0.0, field_dimensions.width / 2.0].framed();
    let right_center_t_junction = point![0.0, -field_dimensions.width / 2.0].framed();
