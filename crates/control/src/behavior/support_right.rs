...
use types::{
    configuration::RolePositions, rotate_towards, BallState, FieldDimensions, FilteredGameState,
...
pub fn execute(
    ...
    let pose = support_right_pose(world_state, field_dimensions, role_positions)?;
    walk_and_stand.execute(pose, look_action.execute(), path_obstacles_output)
}

fn support_right_pose(
    world_state: &WorldState,
    field_dimensions: &FieldDimensions,
    role_positions: &RolePositions,
    ...
    }) * -(Vector2::x() * role_positions.right_midfielder_distance_to_ball);
    ...
    let clamped_x = match world_state.filtered_game_state {
        ...
        | Some(FilteredGameState::Playing {
            ...
            role_positions
                ...
                .min(role_positions.right_midfielder_maximum_x_in_ready_and_when_ball_is_not_free),
            role_positions
                .right_midfielder_minimum_x
                .max(role_positions.right_midfielder_maximum_x_in_ready_and_when_ball_is_not_free),
        ),
        _ => supporting_position.x.clamp(
            role_positions.right_midfielder_minimum_x,
