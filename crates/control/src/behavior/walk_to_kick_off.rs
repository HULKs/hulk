...
use linear_algebra::{point, Pose2, Rotation2};
use types::{
...
pub fn execute(
    ...
    distance_to_kickoff: f32,
    kick_off_angle: f32,
    walk_speed: WalkSpeed,
    distance_to_be_aligned: f32,
) -> Option<MotionCommand> {
    let ground_to_field = world_state.robot.ground_to_field?;
    let kick_off_pose = Rotation2::<Field, Field>::new(-kick_off_angle)
        * Pose2::from(point![-distance_to_kickoff, 0.0]);
