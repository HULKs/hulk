...
pub fn execute(
    ...
    let pose = support_left_pose(world_state, field_dimensions, role_positions)?;
    walk_and_stand.execute(pose, look_action.execute(), path_obstacles_output)
}

fn support_left_pose(
    world_state: &WorldState,
    field_dimensions: &FieldDimensions,
    role_positions: &RolePositions,
    ...
    }) * -(Vector2::x() * role_positions.left_midfielder_distance_to_ball);
    ...
    let clamped_x = match world_state.filtered_game_state {
        ...
        | Some(FilteredGameState::Playing {
            ...
            role_positions
                ...
                .min(role_positions.left_midfielder_maximum_x_in_ready_and_when_ball_is_not_free),
            role_positions
                .left_midfielder_minimum_x
                .max(role_positions.left_midfielder_maximum_x_in_ready_and_when_ball_is_not_free),
        ),
        _ => supporting_position.x.clamp(
            role_positions.left_midfielder_minimum_x,
