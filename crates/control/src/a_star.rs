...
pub trait DynamicMap {
    ...
    fn get_available_exits(&mut self, _idx: usize) -> SmallVec<[(usize, f32); 10]> {
        SmallVec::new()
    }
...
/// DynamicMap implementation), and it requires access to your map so as to call distance and exit determinations.
pub fn a_star_search<T>(start: T, end: T, map: &mut dyn DynamicMap) -> NavigationPath
where
    T: TryInto<usize>,
{
...
impl AStar {
    ...
    /// Wrapper to the DynamicMap's distance function.
    fn distance_to_end(&self, idx: usize, map: &dyn DynamicMap) -> f32 {
        map.get_pathing_distance(idx, self.end)
    }

    /// Adds a successor; if we're at the end, marks success.
    fn add_successor(&mut self, q: Node, idx: usize, cost: f32, map: &dyn DynamicMap) {
    ...
    fn search(&mut self, map: &mut dyn DynamicMap) -> NavigationPath {
...
mod test {
    ...
    impl DynamicMap for TriangleMap {
        fn get_available_exits(&mut self, idx: usize) -> SmallVec<[(usize, f32); 10]> {
            match idx {
    ...
    fn avoid_expensive_shortcut_on_triangle() {
        let mut map = TriangleMap;
        let path = a_star_search(0, 2, &mut map);
        println!("{:?}", path.steps);
    ...
    impl DynamicMap for ExpensiveShortcutMap {
        fn get_available_exits(&mut self, idx: usize) -> SmallVec<[(usize, f32); 10]> {
    ...
    fn avoid_expensive_shortcut() {
        let len = 15;
        let mut map = ExpensiveShortcutMap { len };
        let path = a_star_search(0, len - 1, &mut map);
        println!("{:?}", path.steps);
