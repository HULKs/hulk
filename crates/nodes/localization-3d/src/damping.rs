...
use types::{primary_state::PrimaryState, time_wrapper::TimeWrapper};
...
pub(crate) fn initial_state_for_reset(
    ...
    camera_matrix_cache
        .get_latest()
        .map(|camera_matrix| initial_state_from_camera_matrix(&camera_matrix.inner))
...
pub(crate) async fn reset_and_publish_unlocalized(
    frontend: &mut VinsFrontend,
    live_localization: &mut LiveVisualOdometryLocalization,
    global_visual_lock: &mut GlobalVisualLock,
    initial_state: InitialState,
    ...
    publishers.publish_unlocalized(time).await
}
