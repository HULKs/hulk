...
fn spectrum_contains_whistle(
    ...
    let chunk_size = band_size
        .checked_div(detection_parameters.number_of_chunks)
        .unwrap_or(0);
    ...
    let complete_chunks_end = min_frequency_index + band_values.len() / chunk_size * chunk_size;
    let upper_band_index = complete_chunks_end - upper_whistle_chunk * chunk_size;
