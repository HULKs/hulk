...
/// Generated CDR codec used by types implementing the low-level CDR traits.
...
pub trait WireMessage: Send + Sync + Sized + 'static {
    /// Transport codec used for this wire message.
    type Codec: for<'a> WireEncoder<Input<'a> = &'a Self> + WireDecoder;

    /// Serialize this message to contiguous bytes.
    fn serialize(&self) -> Vec<u8> {
        Self::Codec::serialize(self)
    }

    /// Serialize this message to a Zenoh buffer.
    ...
    /// Deserialize a message from the codec input type.
    fn deserialize(
