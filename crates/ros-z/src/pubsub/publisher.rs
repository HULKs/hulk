...
pub(super) fn validate_dynamic_publish_schema(
    advertised_schema: Option<&Arc<crate::dynamic::MessageSchema>>,
    message: &crate::dynamic::DynamicMessage,
) -> Result<()> {
    let Some(advertised_schema) = advertised_schema else {
        return Ok(());
    };

    if advertised_schema.as_ref() == message.schema() {
        return Ok(());
    }

    Err(zenoh::Error::from(format!(
        "schema mismatch: dynamic message schema '{}' does not match advertised schema '{}'",
        message.schema().type_name_str(),
        advertised_schema.type_name_str()
    )))
}
...
pub struct Publisher<T, C: WireEncoder = <T as crate::Message>::Codec> {
    ...
    dyn_schema: Option<Arc<crate::dynamic::schema::MessageSchema>>,
...
impl<T, C> PublisherBuilder<T, C> {
    ...
    ///
    /// let publisher = node.publisher::<ros_z_msgs::std_msgs::String>("topic")
    ///     .shm_config(config)
    ///     .build()
    ///     .await?;
    /// # Ok(())
    ...
    /// let publisher = node.publisher::<ros_z_msgs::std_msgs::String>("small_messages")
    ///     .without_shm()
    ///     .build()
    ///     .await?;
    /// # Ok(())
    ...
    ///
    /// When a schema is set and the node has a schema service enabled,
    /// the schema will be automatically registered with the service during build.
    /// This allows other nodes to query for this type's schema.
    ///
    /// # Example
    ///
    /// ```ignore
    /// let publisher = node
    ///     .publisher_with_type_info::<DynamicMessage>("topic", None)
    ///     .codec::<DynamicCdrCodec>()
    ///     .dyn_schema(schema)
    ///     .build()
    ///     .await?;
    /// ```
    pub fn dyn_schema(mut self, schema: Arc<crate::dynamic::schema::MessageSchema>) -> Self {
        // Only compute and set type_info if it hasn't been set already
        // (e.g., from dynamic_subscriber_auto which provides the publisher's hash)
    ...
    C: for<'a> WireEncoder<Input<'a> = &'a T> + 'static,
{
    ...
    fn prepare_publish_payload(
        ...
        if let Some(schema) = self.dyn_schema.as_ref()
            && let Some(message) =
                (message as &dyn std::any::Any).downcast_ref::<crate::dynamic::DynamicMessage>()
        {
...
impl Publisher<crate::dynamic::DynamicMessage, crate::dynamic::DynamicCdrCodec> {
