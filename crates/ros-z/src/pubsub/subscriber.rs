    ...
    T: Send + Sync + 'static,
{
    ...
    ///
    /// This is required when using `DynamicMessage` with `DynamicCdrCodec`.
    ...
    /// ```ignore
    /// let subscriber = node
    ///     .subscriber::<DynamicMessage>("/topic")
...
pub struct Subscriber<T, C: WireDecoder = <T as crate::Message>::Codec> {
    ...
    /// Required when using `DynamicMessage` with `DynamicCdrCodec`.
...
impl Subscriber<crate::dynamic::DynamicMessage, crate::dynamic::DynamicCdrCodec> {
    ...
    pub async fn recv(&self) -> Result<crate::dynamic::DynamicMessage> {
        self.recv_with_metadata().await.map(Received::into_message)
    }

    pub async fn recv_with_metadata(&self) -> Result<Received<crate::dynamic::DynamicMessage>> {
        let schema = self
            .dyn_schema
            .as_ref()
