...
//!
//! `ros-z` provides typed pub/sub, services, and actions built directly on
...
//! publisher.publish(&RosString { data: "hello".into() }).await?;
//! ```
//!
//! ## Sync and async APIs
//!
//! Runtime-resource builders (context, node, pub/sub, services, actions, and
...
/// Native ros-z action support (goal, feedback, result).
pub mod action;
...
pub mod graph;
/// Native ros-z lifecycle node support (state machine, lifecycle publisher).
pub mod lifecycle;
...
pub use type_info::{ActionTypeInfo, FieldTypeInfo, ServiceTypeInfo};
pub use zbuf::ZBuf;
