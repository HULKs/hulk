...
use crate::{
    FieldTypeInfo, Message, SerdeCdrCodec,
...
impl Message for Time {
    ...
    fn schema() -> std::sync::Arc<crate::dynamic::MessageSchema> {
        crate::dynamic::MessageSchema::builder("ros_z::Time")
            .field("duration", Duration::field_type())
            .build()
            .expect("failed to build schema for Time")
    }

    fn schema_hash() -> crate::entity::SchemaHash {
        crate::dynamic::schema_hash(&Self::schema()).expect("Time schema must hash")
    }
}
...
impl FieldTypeInfo for Duration {
    fn field_type() -> crate::dynamic::FieldType {
        FieldType::Message(
            MessageSchema::builder("builtin_interfaces::Duration")
...
mod tests {
    ...
    fn ztime_type_info_uses_trait_default_schema_hash() {
        let expected = crate::dynamic::schema_hash(&Time::schema())
    ...
    fn ztime_type_info_uses_native_schema_type_name() {
        ...
        assert_eq!(Time::schema().type_name_str(), "ros_z::Time");
    ...
    fn ztime_duration_field_uses_native_nested_type_name() {
        ...
        let FieldType::Message(duration) = &schema.fields()[0].field_type else {
            panic!("expected nested duration field");
        };

        assert_eq!(duration.type_name_str(), "builtin_interfaces::Duration");
    }
