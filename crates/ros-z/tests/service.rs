...
async fn lifecycle_node_exposes_schema_service_by_default() -> zenoh::Result<()> {
    let context = test_context().await?;
    let node = context.create_lifecycle_node("lc_default").build().await?;

    assert!(node.inner.schema_service().is_some());
    Ok(())
}

#[tokio::test(flavor = "multi_thread", worker_threads = 1)]
async fn lifecycle_node_can_disable_schema_service_explicitly() -> zenoh::Result<()> {
    let context = test_context().await?;
    let node = context
        .create_lifecycle_node("lc_no_schema")
        .without_schema_service()
        .build()
        .await?;

    assert!(node.inner.schema_service().is_none());
    Ok(())
}
