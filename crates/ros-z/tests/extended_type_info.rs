...
use ros_z::{
    ...
    dynamic::{DynamicValue, EnumPayloadValue, message_schema_to_bundle},
};
...
fn extended_derive_keeps_standard_schema_for_compatible_structs() {
    ...
    assert_eq!(schema.type_name_str(), "extended_type_info::TelemetryLite");

    assert!(RobotEnvelope::schema().uses_extended_types());
    assert!(RobotState::schema().uses_extended_types());
}
...
fn extended_derive_generates_distinct_generic_names_and_hashes() {
    ...
    assert_eq!(
        ...
        "extended_type_info::GenericEnvelope__u32"
    );
    assert_eq!(
        GenericEnvelope::<TelemetryLite>::type_name(),
        "extended_type_info::GenericEnvelope__extended_type_info_telemetrylite"
    );
    assert_ne!(
    ...
    let payload = message_schema.field("payload").expect("payload field");
    match &payload.field_type {
        ros_z::dynamic::FieldType::Message(nested) => {
            assert_eq!(nested.type_name_str(), "extended_type_info::TelemetryLite");
        }
...
fn derived_message_hash_matches_manual_runtime_bundle_hash() {
    let runtime = ros_z::dynamic::MessageSchema::builder("extended_type_info::DerivedEnvelope")
        .field(
            "mission_id",
            ros_z::dynamic::FieldType::Optional(Box::new(ros_z::dynamic::FieldType::Uint32)),
        )
        .build()
...
fn extended_derive_keeps_extended_only_generic_instantiations_on_extended_path() {
    ...
    assert_eq!(
        ...
        "extended_type_info::GenericOptionalEnvelope__u32"
    );
...
async fn discovery_uses_schema_service_for_standard_compatible_types() {
    ...
    let registered_hash = ros_z_schema::compute_hash(
        &message_schema_to_bundle(&TelemetryLite::schema()).expect("schema bundle"),
    );
...
async fn extended_only_types_use_schema_service_when_enabled() {
    ...
    let registered_hash = ros_z_schema::compute_hash(
        &message_schema_to_bundle(&RobotEnvelope::schema()).expect("schema bundle"),
    );
    let registered = pub_node
        .schema_service()
    ...
    assert!(schema.uses_extended_types());
    assert_eq!(schema.type_name_str(), "extended_type_info::RobotEnvelope");
