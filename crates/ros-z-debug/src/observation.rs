...
use ros_z::{EndpointGlobalId, Message, dynamic::DynamicPayload, node::Node, time::Time};
...
struct TopicGraphFingerprint {
    publishers: Vec<(EndpointGlobalId, String, String, String)>,
}
...
mod tests {
    ...
    async fn topic_graph_fingerprint_distinguishes_same_type_publisher_identity() {
        let context = ContextBuilder::default().build().await.unwrap();
        let node = context
            .create_node("fingerprint_identity_pub")
            .build()
            .await
            .unwrap();
        let topic = "/42/fingerprint_identity";
        let first_publisher = node.publisher::<String>(topic).build().await.unwrap();
        ...
        wait_for_publisher_count(&node, topic, 0).await;
        ...
        assert_ne!(
            ...
            "same-type publisher identity changes must be relevant graph changes"
        );
    }
