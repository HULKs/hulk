...
async fn setup_node(namespace: &str) -> zenoh::Result<Node> {
...
async fn test_two_streams_boundary_semantics() -> zenoh::Result<()> {
    let node = setup_node("/test_boundary").await?;
    ...
    let mut map = node
        ...
        .create_future_subscriber::<String>("stream/alpha")
        .await?
        .create_future_subscriber::<String>("stream/beta")
        .await?
        .build();
...
async fn test_silent_stream_does_not_block() -> zenoh::Result<()> {
    let node = setup_node("/test_silent").await?;
    ...
    // Since `stream/beta` never announced, its safe time resolves to `None`.
    // The `global_safe_time` becomes `None`, allowing `alpha` to progress directly to persistent.
...
async fn test_interleaved_fusion() -> zenoh::Result<()> {
    let node = setup_node("/test_interleaved").await?;
    ...
    // The `global_safe_time` is bounded by `alpha` at 30, so `beta_30` remains in temporary.
    let item_first = map.recv().await?;
    dbg!(&item_first);
