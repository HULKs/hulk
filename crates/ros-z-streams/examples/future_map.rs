...
async fn main() -> zenoh::Result<()> {
    ...
    let pub_a = node.announcing_publisher::<String>("fusion/a").await?;
    let pub_b = node.announcing_publisher::<String>("fusion/b").await?;
    ...
    let mut map = node
        ...
        .await?
        .create_future_subscriber::<String>("fusion/b", LagPolicy::Immediate)
        .await?
        .build();
    ...
    sender.publish(&"a@10".to_string()).await?;

    for (ts, (a, b)) in &out3.item.persistent {
        println!(
            "ts={:?} a={:?} b={:?}",
            ts,
            a.as_ref().map(|message| message.as_str()),
            b.as_ref().map(|message| message.as_str())
        );
    }
