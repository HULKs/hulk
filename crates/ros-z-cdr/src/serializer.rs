...
#[inline]
fn checked_u32_len(kind: &'static str, len: usize) -> Result<u32> {
    u32::try_from(len).map_err(|_| Error::LengthOverflow { kind, len })
}

#[inline]
fn checked_cdr_string_len(len: usize) -> Result<u32> {
    let len = len.checked_add(1).ok_or(Error::LengthOverflow {
        kind: "string",
        len,
    })?;
    checked_u32_len("string", len)
}
...
impl<'a, BO: ByteOrder> SerdeCdrSerializer<'a, BO, ZBufWriter> {
    ...
    pub fn serialize_zbuf(&mut self, zbuf: &ZBuf) -> Result<()> {
        ...
        self.writer.write_u32(checked_u32_len("bytes", len)?);
    ...
    B: CdrBuffer,
{
    ...
    fn serialize_str(self, v: &str) -> Result<()> {
        self.writer.write_u32(checked_cdr_string_len(v.len())?);
        self.writer.buffer_mut().extend_from_slice(v.as_bytes());
        self.writer.buffer_mut().push(0);
        Ok(())
    }

    #[inline]
    fn serialize_bytes(self, v: &[u8]) -> Result<()> {
        self.writer.write_u32(checked_u32_len("bytes", v.len())?);
        self.writer.buffer_mut().extend_from_slice(v);
        Ok(())
    }
    ...
    fn serialize_seq(self, len: Option<usize>) -> Result<Self::SerializeSeq> {
        match len {
            ...
            Some(elem_count) => {
                self.writer
                    .write_u32(checked_u32_len("sequence", elem_count)?);
                Ok(self)
            }
        }
    }
    ...
    fn serialize_map(self, len: Option<usize>) -> Result<Self::SerializeMap> {
        match len {
            None => Err(Error::UnknownLength),
            Some(elem_count) => {
                self.writer.write_u32(checked_u32_len("map", elem_count)?);
                Ok(self)
            }
        }
    }
