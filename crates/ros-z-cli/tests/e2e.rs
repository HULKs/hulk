...
static NEXT_DOMAIN: AtomicUsize = AtomicUsize::new(1);
...
impl TestDomain {
    fn new() -> Self {
        let sequence = NEXT_DOMAIN.fetch_add(1, Ordering::Relaxed);
        Self(30_000 + (std::process::id() as usize * 100) + sequence)
    }
...
impl TestEnv {
    ...
    fn rosz(&self) -> RoszCommand {
        RoszCommand::new(self.router.endpoint(), self.domain.id())
    }
}
...
impl RoszCommand {
    fn new(router: &str, domain: usize) -> Self {
        Self {
            args: vec![
                "--router".into(),
                router.into(),
...
async fn graph_json_reports_empty_isolated_domain() -> TestResult {
    let env = TestEnv::new();
    let graph = env.rosz().json_command(["graph"]).run_json();

    assert_eq!(graph["domain_id"].as_u64(), Some(env.domain_id() as u64));
    Ok(())
}
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn record_writes_mcap_that_inspect_reports() -> TestResult {
    let env = TestEnv::new();
    ...
    let output_path = env.temp_path("telemetry.mcap");
    let record_output = env
        .rosz()
        .json_command([
            "record",
            fixture.topic.as_str(),
    ...
    assert!(
        record_output
            .iter()
            .any(|event| event["event"] == "record_startup")
    );
    assert!(
        record_output
            .iter()
            .any(|event| event["event"] == "record_final")
    );
    let final_event = record_output
        .iter()
        .find(|event| event["event"] == "record_final")
        .expect("record final event");
    assert!(
        final_event["report"]["total_messages"]
            .as_u64()
            .unwrap_or(0)
            > 0
    );
    ...
    let inspect = env
        .rosz()
        .json_command(["inspect", path_str(&output_path)])
        .run_json();
    assert!(inspect["message_count"].as_u64().unwrap_or(0) > 0);
    assert!(inspect["topics"].as_array().is_some_and(|topics| {
        topics.iter().any(|topic| {
            topic["topic"] == fixture.topic
                && topic["type_name"] == Telemetry::type_name()
                && topic["schema_hash"] == Telemetry::schema_hash().to_hash_string()
                && topic["message_count"].as_u64().unwrap_or(0) > 0
                && topic["source_ids"].as_array().is_some_and(|sources| {
                    sources.iter().any(|source| {
                        source
                            .as_str()
                            .is_some_and(|value| value.starts_with("gid:"))
                    })
                })
        })
    }));
