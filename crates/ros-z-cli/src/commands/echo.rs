...
use ros_z::dynamic::{
    DynamicMessage, DynamicNamedValue, DynamicSubscriber, DynamicValue, EnumPayloadValue,
        ...
        match output_mode {
            OutputMode::Json => {
                let view = EchoMessageView::new(
                    header.topic.clone(),
                    message.schema().type_name_str().to_string(),
...
fn display_schema_hash(schema: &MessageSchema) -> String {
    schema
        .schema_hash()
        .map(|hash| hash.to_hash_string())
        .unwrap_or_else(|| {
            ros_z::dynamic::schema_hash(schema)
                .map(|hash| hash.to_hash_string())
...
mod tests {
    ...
    #[test]
    fn display_schema_hash_falls_back_to_canonical_runtime_hash() {
        let schema = MessageSchema::builder("std_msgs::String")
            .field("data", FieldType::String)
            .build()
            .expect("schema");
