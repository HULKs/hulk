...
use ros2_client::{
    Context, ContextOptions, DEFAULT_PUBLISHER_QOS, DEFAULT_SUBSCRIPTION_QOS, MessageTypeName,
    Name, Node, NodeName, NodeOptions, Publisher, Subscription, ros2::QosPolicies, rustdds::Topic,
};
...
impl BoosterHardwareInterface {
    pub async fn new(
        ...
        let ros_backend = initialize_ros_backend(keep_running.clone(), parameters.dds_domain_id)
            .await
            .wrap_err("failed to initialize ROS backend")?;
        ...
        Ok(Self {
            ...
            low_state_receiver: Mutex::new(ros_backend.low_state_receiver),
            joint_control_sender: ros_backend.joint_control_sender,
            kick_ball_sender: ros_backend.kick_ball_sender,
            odometer_receiver: Mutex::new(ros_backend.odometer_receiver),
            fall_down_state_receiver: Mutex::new(ros_backend.fall_down_state_receiver),
            button_event_msg_receiver: Mutex::new(ros_backend.button_event_msg_receiver),
            remote_controller_state_receiver: Mutex::new(
                ros_backend.remote_controller_state_receiver,
            ),
            rectified_image_receiver: Mutex::new(ros_backend.rectified_image_receiver),
            stereonet_depth_receiver: Mutex::new(ros_backend.stereonet_depth_receiver),
            stereonet_depth_camera_info_receiver: Mutex::new(
                ros_backend.stereonet_depth_camera_info_receiver,
            ),
            image_left_raw_receiver: Mutex::new(ros_backend.image_left_raw_receiver),
            image_left_raw_camera_info_receiver: Mutex::new(
                ros_backend.image_left_raw_camera_info_receiver,
            ),
...
async fn initialize_ros_backend(
    keep_running: CancellationToken,
    dds_domain_id: u16,
) -> Result<RosBackendHandles> {
    let context = Context::with_options(ContextOptions::new().domain_id(dds_domain_id))
        .wrap_err("failed to create ROS context")?;
    let mut node = context
        .new_node(
            NodeName::new("/", "hulk_booster").wrap_err("failed to create node name")?,
            NodeOptions::new(),
        )
    ...
    let joint_control_sender = spawn_publisher_worker::<LowCommand>(
        ...
        keep_running.clone(),
        "/joint_ctrl",
        MessageTypeName::new("booster_interface", "LowCommand"),
    ...
    let remote_controller_state_receiver = spawn_subscription_worker::<RemoteControllerState>(
        ...
        MessageTypeName::new("booster_interface", "RemoteControllerState"),
        &DEFAULT_SUBSCRIPTION_QOS,
    )?;
    ...
    let spinner = node.spinner().wrap_err("failed to create ROS spinner")?;
    spawn_monitored_task(keep_running.clone(), "ros_spinner", async move {
        spinner
            .spin()
            .await
            .map_err(|error| eyre!(error))
            .wrap_err("ROS spinner stopped unexpectedly")
    });
...
fn spawn_subscription_worker<T: DeserializeOwned + Send + Sync + 'static>(
    node: &mut Node,
    keep_running: CancellationToken,
    ...
    let topic = create_topic(node, topic_name, type_name, qos_policy)?;
    let subscription = node
        .create_subscription(&topic, None)
        .wrap_err_with(|| format!("failed to create subscription for `{topic_name}`"))?;
    let (sender, receiver) = latest_channel(topic_name);

    let task_name = topic_name;
    spawn_monitored_task(
        keep_running.clone(),
        task_name,
        forward_subscription(subscription, sender, keep_running),
    );
    Ok(receiver)
}

fn spawn_publisher_worker<T: Send + serde::Serialize + 'static>(
    node: &mut Node,
    keep_running: CancellationToken,
    topic_name: &'static str,
    type_name: MessageTypeName,
    ...
    let publisher = node
        ...
        .wrap_err_with(|| format!("failed to create publisher for `{topic_name}`"))?;
...
fn create_topic(
    ...
    node.create_topic(
        &Name::parse(topic_name)
            .wrap_err_with(|| format!("failed to create ROS topic name for `{topic_name}`"))?,
        type_name,
        qos_policy,
    )
    .wrap_err_with(|| format!("failed to create ROS topic `{topic_name}`"))
}
    ...
    F: Future<Output = Result<()>> + Send + 'static,
{
    tokio::spawn(async move {
        if let Err(error) = fut.await {
            error!("ROS backend task `{task_name}` failed: {error:?}");
            keep_running.cancel();
        }
    });
}
        ...
        tokio::select! {
            ...
            maybe_message = stream.next() => {
                let Some(message) = maybe_message else {
                    bail!("subscription stream ended")
                };
                let (message, _) = message.map_err(|error| eyre!(error))?;
...
impl LowStateInterface for BoosterHardwareInterface {
    fn read_low_state(&self) -> Result<LowState> {
        let message = self
            ...
            .wrap_err("failed to read low state from `/low_state`")?;
        if message.dropped_messages > 0 {
            warn!(
                "dropped {} stale low state messages from `/low_state`",
...
impl LowCommandInterface for BoosterHardwareInterface {
    fn write_low_command(&self, low_command: LowCommand) -> Result<()> {
        self.run_until_cancelled(self.joint_control_sender.send(low_command))?
            .map_err(|_| eyre!("ROS publisher worker for `/joint_ctrl` closed"))?;
        Ok(())
    }
}
...
impl VisualKickInterface for BoosterHardwareInterface {
    fn write_visual_kick(&self, kick: Kick) -> Result<()> {
        self.run_until_cancelled(self.kick_ball_sender.send(kick))?
            .map_err(|_| eyre!("ROS publisher worker for `/kick_ball` closed"))?;
        Ok(())
    }
}
...
impl OdometerInterface for BoosterHardwareInterface {
    fn get_odometer(&self) -> Result<Odometer> {
        let message = self
            ...
            .wrap_err("failed to read odometer from `/odometer_state`")?;
        if message.dropped_messages > 0 {
            warn!(
                "dropped {} stale odometer messages from `/odometer_state`",
                message.dropped_messages
            );
        }
        Ok(message.value)
...
impl FallDownStateInterface for BoosterHardwareInterface {
    fn read_fall_down_state(&self) -> Result<FallDownState> {
        let message = self
            ...
            .wrap_err("failed to read fall down state from `/fall_down`")?;
        if message.dropped_messages > 0 {
            warn!(
                "dropped {} stale fall down state messages from `/fall_down`",
                message.dropped_messages
            );
        }
        Ok(message.value)
...
impl ButtonEventMsgInterface for BoosterHardwareInterface {
    fn read_button_event_msg(&self) -> Result<ButtonEventMsg> {
        let message = self
            ...
            .wrap_err("failed to read button event from `/button_event`")?;
        if message.dropped_messages > 0 {
            warn!(
                "dropped {} stale button event messages from `/button_event`",
                message.dropped_messages
            );
        }
        Ok(message.value)
...
impl RemoteControllerStateInterface for BoosterHardwareInterface {
    fn read_remote_controller_state(&self) -> Result<RemoteControllerState> {
        let message = self
            ...
            .wrap_err("failed to read remote controller state from `/remote_controller_state`")?;
        if message.dropped_messages > 0 {
            warn!(
                "dropped {} stale remote controller state messages from `/remote_controller_state`",
                message.dropped_messages
            );
        }
        Ok(message.value)
...
impl CameraInterface for BoosterHardwareInterface {
    fn read_rectified_image(&self) -> Result<Image> {
        let message = self
            ...
            .wrap_err("failed to read rectified image from `/StereoNetNode/rectified_image`")?;
        if message.dropped_messages > 0 {
            warn!(
                "dropped {} stale rectified image messages from `/StereoNetNode/rectified_image`",
                message.dropped_messages
            );
        }
        Ok(message.value)
    ...
    fn read_stereonet_depth_image(&self) -> Result<Image> {
        let message = self
            ...
            .wrap_err(
                "failed to read stereonet depth image from `/StereoNetNode/stereonet_depth`",
            )?;
        if message.dropped_messages > 0 {
            warn!(
                "dropped {} stale stereonet depth image messages from `/StereoNetNode/stereonet_depth`",
                message.dropped_messages
            );
        }
        Ok(message.value)
    ...
    fn read_stereonet_depth_camera_info(&self) -> Result<CameraInfo> {
        let message = self
            ...
            .wrap_err("failed to read stereonet depth camera info from `/StereoNetNode/stereonet_depth/camera_info`")?;
        if message.dropped_messages > 0 {
            warn!(
                "dropped {} stale stereonet depth camera info messages from `/StereoNetNode/stereonet_depth/camera_info`",
                message.dropped_messages
            );
        }
        Ok(message.value)
    ...
    fn read_image_left_raw(&self) -> Result<Image> {
        let message = self
            ...
            .wrap_err("failed to read left raw image from `/image_left_raw`")?;
        if message.dropped_messages > 0 {
            warn!(
                "dropped {} stale left raw image messages from `/image_left_raw`",
                message.dropped_messages
            );
        }
        Ok(message.value)
    ...
    fn read_image_left_raw_camera_info(&self) -> Result<CameraInfo> {
        let message = self
            ...
            .wrap_err("failed to read left raw camera info from `/image_left_raw/camera_info`")?;
        if message.dropped_messages > 0 {
            warn!(
                "dropped {} stale left raw camera info messages from `/image_left_raw/camera_info`",
                message.dropped_messages
            );
        }
        Ok(message.value)
