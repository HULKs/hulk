...
impl TryFrom<RoboCupGameControlData> for GameControllerStateMessage {
    ...
    fn try_from(message: RoboCupGameControlData) -> anyhow::Result<Self> {
        ...
        Ok(GameControllerStateMessage {
            ...
            game_state: message.state.try_into()?,
            set_play: SetPlay::try_from(message.kickingTeam, message.setPlay)?,
...
#[derive(Clone, Debug, Deserialize, Serialize)]
pub enum SetPlay {
    GoalKick { kicking_team: Team },
    PushingFreeKick { kicking_team: Team },
    CornerKick { kicking_team: Team },
    KickIn { kicking_team: Team },
    PenaltyKick { kicking_team: Team },
}

impl SetPlay {
    fn try_from(kicking_team: u8, set_play: u8) -> anyhow::Result<Option<Self>> {
        let kicking_team = if kicking_team == HULKS_TEAM_NUMBER {
            Team::Hulks
        } else {
            Team::Opponent
        };
        match set_play {
            ...
            SET_PLAY_GOAL_KICK => Ok(Some(SetPlay::GoalKick { kicking_team })),
            SET_PLAY_PUSHING_FREE_KICK => Ok(Some(SetPlay::PushingFreeKick { kicking_team })),
            SET_PLAY_CORNER_KICK => Ok(Some(SetPlay::CornerKick { kicking_team })),
            SET_PLAY_KICK_IN => Ok(Some(SetPlay::KickIn { kicking_team })),
            SET_PLAY_PENALTY_KICK => Ok(Some(SetPlay::PenaltyKick { kicking_team })),
