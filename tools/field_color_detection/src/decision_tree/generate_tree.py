...
def convert_pixels_BGR2bgrI(pixel_BGR):
    ...
    image_bgI = np.concatenate((chromaticity, intensity), axis=-1)
    return image_bgI
...
def optimize_thresholds(pixels_BGR, pixels_YCrCb, y, camera):
    ...
    pixels_HSV = cv2.cvtColor(pixels_BGR.reshape(1, -1, 3), cv2.COLOR_BGR2HSV).squeeze(
        axis=0
    )

    X = np.concatenate((pixels_BGR, pixels_YCrCb, pixels_bgrI, pixels_HSV), axis=1)
...
def tree_to_rust_code(
    ...
    def recurse(node: int) -> str:
        if tree.children_left[node] == -1 and tree.children_right[node] == -1:
            ...
            return (
                f"if features.{features[feature_index].identifier} <= {threshold_str} {{\n"
                f"{left_branch}\n"
                f"}} else {{\n"
                f"{right_branch}\n"
                f"}}"
...
def save_tree_as_dot_file(model, filename):
    feature_names = ["B", "G", "R", "Y", "Cr", "Cb", "b", "g", "r", "I", "H", "S", "V"]
    export_graphviz(model, f"{filename}.dot", feature_names=feature_names)


def predict_image(image_CrCbY, model):
    ...
    pred = (
        cv2.cvtColor((np.resize(pred, (M, N))).astype(np.uint8), cv2.COLOR_GRAY2BGR)
        * 255
    )
