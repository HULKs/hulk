...
class Config(qtw.QWidget, _LayerConfig):
    def __init__(self,
        ...
        self.config_to_ui = {
            ...
            "position": (
                lambda: {
                    "key": self.cbx_PositionKey.currentText(),
                    "key_lambda": self.edit_PositionKeyLambda.toPlainText(),
                    "circleDiameter":
                        self.spin_positionCircleDiameter.value(),
                    "color": self.edit_positionColor.text()},
                lambda config: [
                    self.cbx_PositionKey.setCurrentText(
                        config["key"]),
                    self.edit_PositionKeyLambda.setPlainText(
                        config["key_lambda"]),
                    self.spin_positionCircleDiameter.setValue(
                        config["circleDiameter"]),
                    ui_utils.reset_textField_color(
                        self.edit_positionColor, config["color"])]
            )
        }
        ...
        self.cbx_PositionKey.completer().setFilterMode(
            qtc.Qt.MatchContains)
        self.cbx_PositionKey.completer().setCompletionMode(
            qtw.QCompleter.PopupCompletion)
        ui_utils.init_Color_UI(
            self.btn_positionColor,
            self.edit_positionColor)
    ...
    def connect(self, nao: Nao):
        ...
        self.nao.debug_protocol.subscribe_msg_type(
            ...
            self.layer_model["identifier"],
            self.fill_cbx)

    def closeEvent(self, event):
        if self.nao.is_connected():
            self.nao.debug_protocol.unsubscribe_msg_type(
                net_utils.DebugMsgType.list, self.layer_model["identifier"])
    ...
    def fill_cbx(self):
        ui_utils.init_cbx(
            ...
            self.nao.debug_data)
        ui_utils.init_cbx(
            self.cbx_PositionKey,
            self.layer_model["config"]["position"]["key"],
            self.nao.debug_data)
