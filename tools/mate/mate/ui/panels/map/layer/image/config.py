...
class Config(qtw.QWidget, _LayerConfig):
    ...
    def fill_cbx(self):
        # ui_utils.init_cbx(
        #     self.cbx_Key,
        #     self.layer_model["config"]["subscribe_key"],
        #     self.nao.debug_data)
        self.cbx_Key.clear()
        for key, data in self.nao.debug_data.items():
            if data.isImage:
                self.cbx_Key.addItem(key)
