#[path = "panels/behavior_tree/control_flow_layout.rs"]
mod control_flow_layout;
#[path = "panels/behavior_tree/graph.rs"]
mod graph;
#[path = "panels/behavior_tree/model.rs"]
mod model;
#[path = "panels/behavior_tree/tree_layout.rs"]
mod tree_layout;

use std::collections::{HashMap, HashSet};

use coordinate_systems::World;
use eframe::egui::{Color32, ComboBox, Response, Stroke, Ui, pos2};
...
pub struct BehaviorTreeVisualizer {
...
impl Default for BehaviorTreeVisualizer {
    fn default() -> Self {
        Self {
            tree_layout: None,
...
impl BehaviorTreeVisualizer {
    pub fn show(
        ...
        self.update_layout_if_needed(tree_layout);
        self.show_controls(ui);
        self.update_trace_strokes(trace);
        self.show_graph(ui)
    }
    ...
    pub fn clear(&mut self) {
        ...
        self.initial_collapse_applied = false;
    }
    ...
    fn show_graph(&mut self, ui: &mut Ui) -> Response {
                ...
                .map(|node| (node.id.clone(), node.animated_position))
        {
            ...
            if let Some(new_pos) = self
                .circle_nodes
                .iter()
                .find(|node| node.id == clicked_subtree_id)
    ...
    fn update_layout_if_needed(&mut self, tree_layout: Option<&NodeTrace>) -> bool {
        ...
        {
            self.tree_layout = Some(tree_layout.clone());
            self.rebuild_layout(None);
    ...
    fn update_trace_strokes(&mut self, trace: Option<&NodeTrace>) {
        for node in &mut self.circle_nodes {
