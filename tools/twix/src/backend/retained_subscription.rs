...
const DYNAMIC_SUBSCRIPTION_QUEUE_DEPTH: usize = 1;
...
pub fn subscribe_dynamic(
    ...
    runtime.spawn(run_subscription(SubscriptionTask {
        ...
        state,
        qos: high_rate_qos(DYNAMIC_SUBSCRIPTION_QUEUE_DEPTH),
...
fn handle_update<T>(
    ...
    match update {
        ...
        SubscriptionUpdate::Diagnostic(message) => {
            state.lock().diagnostic = Some(message);
        }
...
async fn wait_for_rebuild_signal(
    ...
    tokio::select! {
        ...
        changed = graph_changes.changed() => changed.ok().map(|()| RebuildSignal::GraphChanged),
    }
}
...
fn high_rate_qos(queue_depth: usize) -> QosProfile {
    QosProfile {
        reliability: QosReliability::BestEffort,
        history: QosHistory::KeepLast(
            NonZeroUsize::new(queue_depth).expect("high-rate QoS depth must be non-zero"),
        ),
        ..Default::default()
    }
}
...
mod tests {
    ...
    #[tokio::test(flavor = "multi_thread", worker_threads = 2)]
    async fn typed_subscription_exposes_latest_window_and_diagnostics() {
        let topic = "twix_typed_subscription_wrapper";
        ...
        let publisher = publisher_node
            .publisher::<SubscribeErrorStateMessage>(topic)
        ...
        let retention =
            RetentionPolicy::time_window(Duration::from_secs(2)).expect("valid retention window");
        let handle = manager
        ...
        let typed = TypedSubscription { retained };

        publisher
            .publish(&SubscribeErrorStateMessage {
        ...
        loop {
            if let Some(record) = typed.latest() {
                assert_eq!(record.value.value, "latest");
                break;
            }
            assert!(
                tokio::time::Instant::now() < deadline,
                "timed out waiting for typed retained sample"
            );
            tokio::time::sleep(Duration::from_millis(10)).await;
        }

        let window = typed.window(Time::zero(), Time::from_nanos(i64::MAX));
        assert_eq!(window.len(), 1);
        assert_eq!(window[0].value.value, "latest");

        typed.retained.state.lock().diagnostic = Some("decode failed".to_string());
        assert_eq!(typed.diagnostic_message().as_deref(), Some("decode failed"));
    }
