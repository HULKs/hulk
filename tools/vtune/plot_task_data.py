...
parser.add_argument('--supress-wait-modules',
...
if (os.environ.get('VTUNE_HOME')):
  base_project_path = os.path.join(os.environ['VTUNE_HOME'], '..', 'amplxe', 'projects')
...
project_name = os.path.splitext([f for f in os.listdir(absolute_project_path) if 'amplxe' in f][0])[0]
...
if args.output == 'plot':
  ...
  pandas_brain_top_data = pandas.DataFrame(dict([ (k,pandas.Series(v)) for k,v in brain_top_data.items() ]))
  pandas_brain_bot_data = pandas.DataFrame(dict([ (k,pandas.Series(v)) for k,v in brain_bot_data.items() ]))
  pandas_motion_data = pandas.DataFrame(dict([ (k,pandas.Series(v)) for k,v in motion_data.items() ]))

  # Plot Brain Top Image Data
  make_boxplot(1, pandas_brain_top_data, "Brain Top Image Modules\n" + project_name, "ms/cycle")

  # Plot Brain Bottom Image Data
  make_boxplot(2, pandas_brain_bot_data, "Brain Bottom Image Modules\n" + project_name, "ms/cycle")
