...
Map.prototype.onUpdate = function (objects) {
	...
	this.desiredStep = objects.desiredStep;
	this.targetPose = objects.targetPose;
...
Map.prototype.drawDesiredStep = function (style, scale) {
	//for scaling, make relative coord 5 times
	var scaledPose = [[0, 0], 0];
	scaledPose[0][0] = this.desiredStep[0][0] * scale;
	scaledPose[0][1] = this.desiredStep[0][1] * scale;
	scaledPose[1] = this.desiredStep[1];

	var absDesiredStep = this.getAbsolutePose(scaledPose);
	var startCoordPixel = this.getPixelCoordinates(this.getVectorFromPose(this.pose));
	var endCoordPixel = this.getPixelCoordinates(this.getVectorFromPose(absDesiredStep));
...
Map.prototype.drawTarget = function (color) {
	var absTargetPose = this.getAbsolutePose(this.targetPose);
	...
	this.drawArrow(ownPose[0],
		ownPose[1],
		targetPose[0],
		targetPose[1],
		color, 0, 1); //arrow without head!
};
