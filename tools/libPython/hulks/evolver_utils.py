...
class Population(object):
    def  __init__(self,
        ...
        self.samples_cnn_check = samples_cnn_check
        self.input_shape = (self.nn_settings["input_size_x"],
                            self.nn_settings["input_size_y"],
                            self.nn_settings["input_channels"])
        self.datagen = tf.keras.preprocessing.image.ImageDataGenerator(brightness_range=(0.5,1.2))
        self.datagen.fit(self.samples_train)
    ...
    def add_member(self, new_member: Individual, save_files=False):
        ...
        if os.path.isfile(self.trained_model_path + new_member.name + "_" + new_member.name + "_" + str(self.flattened_n(new_member.genes)) + "_" + self.nn_settings["type"] + ".hdf5"):
            load_trained_models = True

        model = None

        if load_trained_models:
            if self.nn_settings["metric"] == "f1":
                model = tf.keras.models.load_model(self.trained_model_path + new_member.name + "_" + self.nn_settings["type"] + ".hdf5",
                                                   custom_objects={"f1": hulks.nn_utils.f1})
            elif self.nn_settings["metric"] == "f2":
                model = tf.keras.models.load_model(self.trained_model_path + new_member.name + "_" + self.nn_settings["type"] + ".hdf5",
                                                   custom_objects={"f2": hulks.nn_utils.f2})
            else:
                model = tf.keras.models.load_model(self.trained_model_path + new_member.name + "_" + self.nn_settings["type"] + ".hdf5")
        else:
            model = tf.keras.models.Sequential(hulks.nn_utils.conv_layer_list(new_member.genes, self.input_shape) +
                                               hulks.nn_utils.dense_layer_list(new_member.genes, self.flattened_n(new_member.genes), self.nn_settings["final_layer_neurons"]))
        try:
            ...
            if self.nn_settings["metric"] == "f1":
                model.compile(
                    ...
                    metrics=[hulks.nn_utils.f1])
            elif self.nn_settings["metric"] == "f2":
                model.compile(
                    optimizer=opt,
                    loss=self.nn_settings["loss"],
                    metrics=[hulks.nn_utils.f2])
            else:
                model.compile(
                    optimizer=opt,
                    loss=self.nn_settings["loss"],
            ...
            hulks.nn_utils.add_to_log("tfkeras compile failed",
                                        {"type": self.nn_settings["type"],
                                         "genes": new_member.genes},
                                        "evolver_tfkeras_fail_log.jsonl")
            return
        new_member.cost = hulks.nn_utils.cost(new_member.genes, self.nn_settings["final_layer_neurons"], self.nn_settings["cost_factor"])
        ...
        if not load_trained_models:
            try:
                ...
                for ep in range(new_member.genes["trainingEpochs"]):
                    ...
                    model.fit_generator(self.datagen.flow(self.samples_train,
                                                     self.labels_train,
                                                     batch_size=32),
                                        steps_per_epoch = len(self.samples_train) / 32,
                                        epochs=1,
                ...
                print(new_member.genes)
                hulks.nn_utils.add_to_log("tfkeras fit failed",
                                          {"type": self.nn_settings["type"],
                                           "genes": new_member.genes},
                                          "evolver_tfkeras_fail_log.jsonl")
                return
        self.spam_pad.print("")
        result = model.evaluate(self.samples_test, self.labels_test, verbose = 0)
        new_member.score = result[1]
        
        tf.keras.models.save_model(model,
                                   self.trained_model_path + "tmp_model.hdf5",
                                   save_format='h5')
        
        new_member.fitness = new_member.score - new_member.cost
        append = True
        distance = hulks.nn_utils.compiledNN_distance(model,
                                                      self.trained_model_path + "tmp_model.hdf5",
                                                      self.samples_cnn_check)
        compiledNN_success = False
        if distance < 0.1 * self.nn_settings["final_layer_neurons"]:
            compiledNN_success = True
        self.status_pad.print(str(compiledNN_success) + " (" + str(distance)[:5].rjust(5," ") +") " + str(new_member.score)[:5].rjust(5," ") + " " + str(new_member.cost)[:5].rjust(5," "))
        ...
        if save_files and not load_trained_models:
            with open(self.population_path + new_member.name + '.json', 'w') as outfile:
                json.dump(dict(new_member.genes), outfile)
            tf.keras.models.save_model(model,
                                       self.trained_model_path + new_member.name + "_" + self.nn_settings["type"] + ".hdf5",
                                       save_format='h5')
        if compiledNN_success:
            self.members.append(new_member)
            hulks.nn_utils.add_to_log("add to population",
                                      {"name": new_member.name,
                                       ...
                                       "genes": new_member.genes},
                                      "evolver_success_log.jsonl")
        else:
            self.failed_counter += 1
            hulks.nn_utils.add_to_log("CompiledNN Fail",
                                      {"name": new_member.name,
                                       "type": self.nn_settings["type"],
                                       "score": float(new_member.score),
                                       "fitness:": float(new_member.fitness),
                                       ...
                                       "tf_cnn_distance": distance,
                                       "genes": new_member.genes},
                                      "evolver_compiledNN_fail_log.jsonl")
            if self.failed_counter < self.save_failed_max:
                tf.keras.models.save_model(model,
                                           self.trained_model_path + "failed/" + new_member.name + "_" + self.nn_settings["type"] + ".hdf5",
                                           save_format='h5')
        os.remove(self.trained_model_path + "tmp_model.hdf5")
