...
parser = argparse.ArgumentParser(
    description="Generate speech from text with google apis"
)
parser.add_argument(
    "-a", "--api_key", required=api_key_needed, help="Google api key"
)
parser.add_argument(
    "-l", "--list-voices", action="store_true", help="List voices"
)
...
if not args.list_voices and (
    args.voice_name is None or len(args.text) == 0 or args.output is None
):
    parser.error(
        "--voice_name, --output and text is required to synthesize text."
    )
...
if args.list_voices:
    ...
    body = {
        "input": {"text": " ".join(args.text)},
        "voice": {"languageCode": args.voice_name[:5], "name": args.voice_name},
