...
pub fn read_recording(path: &Path) -> Result<AnalysisInput> {
    ...
    for message in stream {
        ...
        match relevant_topic {
            ...
            RelevantTopic::DetectedObjects => record_decode_health::<
                TimeWrapper<Vec<Object<RobocupObjectLabel>>>,
            >(
                &message, topic, time_ns, &mut output.decode_errors
            ),
...
fn record_decode_health<T>(
    ...
    T: DeserializeOwned,
{
    if let Err(error) = decode::<T>(message) {
        decode_errors.push(decode_error(topic, time_ns, error));
    }
}
...
mod tests {
    ...
    use super::{RelevantTopic, read_recording, relevant_topic};
