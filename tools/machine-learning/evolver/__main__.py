...
available_settings = {
    "preclassifier": {
        ...
        "optimizers": ["Adadelta", "Adam", "Adamax", "Nadam", "RMSprop", "SGD"],
    },
    "classifier": {
        "type": "classifier",
        "final_layer_neurons": 1,
...
def parse_arguments():
    arg_parser = argparse.ArgumentParser(
        description="Neural Net Evolver",
...
def main():
    ...
    trained_model_path = data_path / (
        "trained_" + settings["type"] + "_models/"
    )
        ...
        for filename in population_path.iterdir():
            if filename.suffix == ".json":
                ...
                try:
                    ...
                    data = json.load(f)
                    f.close()
                    new_individual = evolver_utils.Individual(
                        name=filename.name[0:-5], genes=data
                    )
...
def curses_main(stdscr):
    ...
    progression_pad = ui.ProgressionPad(
        from_line=8 + popmax, to_line=38 + popmax, stdscr=stdscr
    )
    stdscr.clear()
    main()
