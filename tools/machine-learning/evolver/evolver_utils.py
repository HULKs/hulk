...
class Population(object):
    def __init__(
        ...
        self.tfr_train = nn_utils.get_dataset(
            self.tfr_train_ds,
            self.batch_size,
        ...
        self.tfr_test = nn_utils.get_dataset(
            self.tfr_test_ds,
            self.batch_size,
            self.nn_settings["type"],
            self.nn_settings["input_size_x"] * self.nn_settings["input_size_y"],
        ...
        self.tfr_verify = nn_utils.get_dataset(
            self.tfr_verify_ds,
        ...
        self.evolver_tfkeras_fail_log = (
            f"evolver_tfkeras_fail_log_{self.evolver_run_uuid}.jsonl"
        )
        self.evolver_success_log = (
            f"evolver_success_log_{self.evolver_run_uuid}.jsonl"
        )
        self.evolver_compiledNN_fail_log = (
            f"evolver_compiledNN_fail_log_{self.evolver_run_uuid}.jsonl"
        )
        ...
        cnn_samples = [
            flatten(flatten(sample)) for sample in self.tfr_verify_cnn_ds
        ]
    ...
    class PrintCallback(tf.keras.callbacks.Callback):
        ...
        def on_epoch_begin(self, epoch, logs=None):
            self.pad.print(
                "Epoch: " + str(self.ep) + " of " + str(self.eps) + "\n"
            )
    ...
    def add_member(self, new_member: Individual, save_files=False):
        ...
        if os.path.isfile(
            str(
                ...
                / (
                    ...
                    + new_member.name
                    + "_"
                    + str(self.flattened_n(new_member.genes))
                    + "_"
                    + self.nn_settings["type"]
                    + ".hdf5"
                )
        ...
        if load_trained_models:
            ...
            full_layer_list = nn_utils.conv_layer_list(
                new_member.genes, self.input_shape
            )
            full_layer_list.extend(
                nn_utils.dense_layer_list(
                    new_member.genes,
                    self.flattened_n(new_member.genes),
                    self.nn_settings["final_layer_neurons"],
                )
            )
        ...
        try:
            ...
            nn_utils.add_to_log(
                "tfkeras compile failed",
                {"type": self.nn_settings["type"], "genes": new_member.genes},
                self.evolver_tfkeras_fail_log,
            )
            return
        new_member.cost = nn_utils.cost(
            new_member.genes,
            self.nn_settings["final_layer_neurons"],
            self.nn_settings["cost_factor"],
        )
        print("cost : ", new_member.cost)
        print(new_member.genes)
        model.summary()
        self.member_pad.print(
            new_member,
            self.nn_settings["type"],
            self.nn_settings["final_layer_neurons"],
        )
        ...
        if not load_trained_models:
            try:
                ...
                for ep in range(new_member.genes["trainingEpochs"]):
                    printCallback = self.PrintCallback(
                        self.spam_pad,
                        ep + 1,
                        new_member.genes["trainingEpochs"],
                    )
                    model.fit(
                        self.tfr_train,
        ...
        result = model.evaluate(
            self.tfr_test, verbose=0
        )  # steps = self.nn_settings["batches_for_scoring_test"], verbose = 0)
        ...
        append = True
        distance = nn_utils.compiledNN_average_distance(
            model,
            str(self.trained_model_path / self.temp_model_file),
            self.tfr_verify,
            self.cnn_sample_string,
        ...
        self.status_pad.print(
            str(compiledNN_success)
            + " ("
            + "%.4f" % distance
            + ") "
            + str(new_member.score)[:5].rjust(5, " ")
            + " "
            + str(new_member.cost)[:5].rjust(5, " ")
        )
        if (
            new_member.fitness > self.best_fitness.fitness
        ) and compiledNN_success:
        ...
        if save_files and not load_trained_models:
            with open(
                str(self.population_path / (new_member.name + ".json")), "w"
            ) as outfile:
    ...
    def add_random_member(self):
        ...
        for i in range(hiddenLayers):
            ...
            dropout = random.random() / 2.0
            thisDLayers.append(
                (neurons, activation_function, batchnorm, dropout)
            )
        randomName = ""
        ...
        self.add_member(
            Individual(
                name=randomName + "-" + str(self.generation),
                genes={
                    "trainingEpochs": random.randint(1, 4),
                    "conv_layers": thisCLayers,
                    "dense_layers": thisDLayers,
                    "final_layer": (
                        random.choice(self.final_activation_functions),
            ...
            if mutationtype == 0:
                ...
                if mut == 0:  # remove layer
                    ...
                    dlay[layer_index] = (
                        random.randint(
                            math.floor(
                                2 + math.floor(dlay[layer_index][0] / 2)
                            ),
                            2 * (dlay[layer_index][0]),
                        ),
                        random.choice(self.dense_activation_functions),
                        random.choice([True, False]),
                        random.random() / 2.0,
                ...
                if mut == 0:
                    ...
                    clay[layer_index] = (
                        clay_type,
                        random.choice([4, 8, 16, 32, 64, 128]),
                        clay_size,
                        clay_act,
                        clay_m,
                        clay_bn,
                    ...
                    clay[layer_index] = (
                        ...
                        clay_kernels,
                        clay_size,
                        random.choice(self.conv_activation_functions),
                        clay_m,
                        clay_bn,
                        clay_dr,
                        clay_s,
                    ...
                    clay[layer_index] = (
                        ...
                        clay_m,
                        clay_bn,
                        min(clay_dr * (0.5 + random.random()), 0.5),
                        clay_s,
                    )
                elif mut == 7:
                    if size > 2:
                ...
                if add:
                    ...
                    activation_function = random.choice(
                        self.conv_activation_functions
                    )
                    maxpool = random.choice([2, -2, 0])
                    if maxpool != 0:
    ...
    def cull(self):
        ...
        while len(self.members) > self.min_size and random.choice(
            [True, False]
        ):
            self.remove_worst()
            self.pop_pad.print(self)
    ...
    def evolve(self):
        ...
        if len(self.members) < self.min_size:
            self.status_pad.print(
                "Create randoms until pop-minsize("
                + str(self.min_size)
                + ") is reached"
            )
