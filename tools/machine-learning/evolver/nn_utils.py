...
def f_beta(beta, y_true, y_pred):
    ...
    beta_sq = beta * beta
    return (1 + beta_sq) * (
        (precision * recall) / ((beta_sq * precision) + recall + K.epsilon())
    )


def compiledNN_average_distance(
    model, model_path, samples, cnn_sample_string, predicter
):
    counter = 0
    ...
    for i in range(len(tf2_keras_results)):
        for j in range(len(tf2_keras_results[i])):
            try:
                distance += abs(
                    float(tf2_keras_results[i][j])
                    - float(compiledNN_output[i][j])
                )
                value_counter += 1
    ...
    for t, n, k, a, m, bn, dr, s in genes["conv_layers"]:
        if is_first_clayer:
            ...
            if t == "Conv2D":
                ...
                result.append(
                    tf.keras.layers.SeparableConv2D(
                        n, (k, k), padding="same", strides=s, use_bias=False
                    )
                )
        if is_first_clayer:
        ...
        if m != 0:
            if m > 0:
                ...
                result.append(
                    tf.keras.layers.AveragePooling2D(pool_size=(-m, -m))
                )
        if dr > 0.01:
...
def parse_tfrecord_class(data_size, augment_params, example):
    ...
    return (
        shape_and_augment_sample(
            parsed["data"], parsed["dataShape"], augment_params
        ),
...
def parse_tfrecord_circle(data_size, augment_params, example):
    ...
    return (
        ...
        tf.math.multiply(
            parsed["circle"], tf.constant([1.0 / 32.0, 1.0 / 32.0, 1.0 / 16.0])
        ),
    )
...
def get_dataset(tfr_ds, batch_size, nnType, data_size, augment_params):
    if nnType == "positioner":
        tfr_ds = tfr_ds.map(
            lambda x: parse_tfrecord_circle(data_size, augment_params, x)
        )
        ...
        tfr_ds = tfr_ds.map(
            lambda x: parse_tfrecord_class(data_size, augment_params, x)
        )
        # tfr_ds = tfr_ds.repeat()
        tfr_ds = tfr_ds.batch(batch_size)
        tfr_ds = tfr_ds.prefetch(batch_size)
