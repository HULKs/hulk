...
def make_input(cfg, spec):
    ...
    imgs = (list(valid_dir.glob("*.png")) + list(valid_dir.glob("*.jpg"))) if valid_dir.exists() else []
...
def main() -> None:
    ...
    if "CUDAExecutionProvider" in available:
        ...
        if not cuda_active:
            parity_line = f"CUDA EP listed but did NOT bind (active={active}) - GPU parity NOT validated"
        else:
            ...
            parity_line = (f"CPU vs CUDA(fp32) max abs diff = {max_diff:.3e} "
    ...
    lines = [
        ...
        *[f"  {i}: {n}" for i, n in enumerate(cfg.model.class_names)],
        "", "## Inputs", *[f"  {s.name}: {s.shape} {s.type}" for s in in_specs],
        "", "## Outputs", *[f"  {s.name}: {s.shape} {s.type}" for s in out_specs],
    ...
    print(f"CPU finite={cpu_finite} CUDA bound={cuda_active} finite={gpu_finite} shapes_ok={shapes_ok}")
    print(parity_line)
