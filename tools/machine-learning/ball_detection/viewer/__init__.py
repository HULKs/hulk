...
app = flask.Flask(
    __name__, static_folder="static", static_url_path="/viewer/static"
)
...
def masked_crc32c(data: bytes):
    ...
    return (
        (((checksum >> 15) & 0xFFFFFFFF) | ((checksum << 17) & 0xFFFFFFFF))
        + k_mask_delta
    ) & 0xFFFFFFFF
...
def sample(index):
    ...
    with app.config["input_samples_file_lock"]:
        ...
        masked_crc32_of_data = struct.unpack("<L", masked_crc32_of_data_bytes)[
            0
        ]
    ...
    data = np.reshape(
        np.array(data_feature.int64_list.value, dtype=np.uint8),
        tuple(data_shape),
    )
...
def server(*args, **kwargs):
    ...
    while position < size:
        ...
        masked_crc32_of_length = struct.unpack(
            "<L", masked_crc32_of_length_bytes
        )[0]
