...
def generate_random_sample_positions(config, random_generator, image, circle):
    sample_size = np.array([[config["sample_size"]], [config["sample_size"]]])
    annotation_box_upper_left = np.array(
        [
            [circle["centerX"] - circle["radius"]],
            [circle["centerY"] - circle["radius"]],
        ]
    )
    annotation_box_size = np.array(
        [[2 * circle["radius"]], [2 * circle["radius"]]]
    )
    annotation_box_size_x = np.array([[2 * circle["radius"]], [0]])
    ...
    scale_factor = random_generator.uniform(
        config["minimum_sample_scale_factor"],
        config["maximum_sample_scale_factor"],
    )
    translation_factor = np.array(
        [
            [
                random_generator.uniform(
                    -config["maximum_sample_translation_factor"],
                    config["maximum_sample_translation_factor"],
                )
            ],
            [
    ...
    def translate2d(t):
        return np.array([[1, 0, t[0, 0]], [0, 1, t[1, 0]], [0, 0, 1]])

    def scale2d(s):
        return np.array([[s[0, 0], 0, 0], [0, s[1, 0], 0], [0, 0, 1]])
        ...
        np.matmul(
            ...
            np.matmul(
                ...
                np.matmul(
                    ...
                    translate2d(
                        -annotation_box_upper_left - (annotation_box_size / 2)
                    ),
                ),
            ),
        ),
    ...
    return (
        circle_center,
        circle_radius,
        to2d(transformed_circle_to_sample).astype(np.float),
    )
...
def sample_intersection_ratio(config, image, circle, transform):
    ...
    image_lower_right = np.array([[image.shape[1]], [image.shape[0]]])
    ...
    intersected_amount = np.count_nonzero(
        (x >= image_upper_left[0, 0])
...
@click.option(
    "--minimum-sample-scale-factor",
    default=1.35,
    help="Minimum sample scale factor for annotations",
    show_default=True,
)
@click.option(
    "--maximum-sample-scale-factor",
    default=1.85,
    help="Maximum sample scale factor for annotations",
    show_default=True,
)
@click.option(
    "--maximum-sample-translation-factor",
    default=0.6,
    help="Maximum sample translation factor for annotations",
    show_default=True,
)
@click.option(
    "--maximum-sample-rotation",
    default=30,
    help="Maximum sample rotation for annotations (degree)",
    show_default=True,
)
@click.option(
    "--samples-per-annotation",
    default=15,
    help="Samples per annotation",
    show_default=True,
)
@click.option(
...
@click.option(
    "--maximum-attempts-factor-per-annotation",
    default=5,
    help="The maximum attempts factor per annotation",
    show_default=True,
)
@click.option(
    "--label-type",
    ...
    default="positive",
    help="Type of label of generated samples",
    show_default=True,
)
@click.option(
    "--intersection-test-point-amount",
    default=128,
    help="Test point amount for intersection ratio approximation",
    show_default=True,
)
@click.option(
    "--intersection-ratio-threshold",
    default=0.25,
    help="Threshold for intersection ratio",
    show_default=True,
)
@click.option(
    "--avoidance-annotations-file",
    type=click.File(mode="r"),
    help="Annotations file containing circles to avoid",
)
@click.option(
    "--avoidance-intersection-test-point-amount",
    default=128,
    help="Test point amount for avoidance intersection ratio approximation",
    show_default=True,
)
@click.option(
    "--avoidance-intersection-ratio-threshold",
    default=0,
    help="Threshold for intersection ratio",
    show_default=True,
)
...
def main(**config):
    config["avoidance_annotations"] = (
        json.load(config["avoidance_annotations_file"])
        if config["avoidance_annotations_file"] is not None
        else {}
    )
    ...
    with click.progressbar(annotations.items()) as annotation_items:
        for image_path, circles in annotation_items:
            ...
            for circle in circles:
                ...
                for _ in range(
                    int(
                        config["samples_per_annotation"]
                        * config["maximum_attempts_factor_per_annotation"]
                    )
                ):
                    if valid_samples_found >= config["samples_per_annotation"]:
                        break

                    circle_center, circle_radius, transform = (
                    ...
                    if (
                        avoidance_intersection_ratio(
                            config, image_path, transform
                        )
                        > config["avoidance_intersection_ratio_threshold"]
                    ):
                        continue

                    if (
                        sample_intersection_ratio(
                            config, image, circle, transform
                        )
                        < config["intersection_ratio_threshold"]
                    ):
                        continue
