        ...
        if config["mode"] == "b-alls-2019":
            ...
            for kind in ["positives", "negatives"]:
                ...
                with click.progressbar(
                    enumerate(zip(hdf5[kind]["data"], hdf5[kind]["labels"])),
                    length=len(hdf5[kind]["data"]),
                    label=f"{config['input_hdf5_file'].name} ({kind})",
                ) as examples:
                    for i, (data, labels) in examples:
                        filename = f"{i:06d}.png"
                        cv2.imwrite(
                            os.path.join(kind_directory, filename), data
                        )
            ...
            for event in hdf5.keys():
                ...
                with click.progressbar(
                    enumerate(
                        zip(hdf5[event]["images"], hdf5[event]["labels"])
                    ),
                    length=len(hdf5[event]["images"]),
                    label=f"{config['input_hdf5_file'].name} ({event})",
                ) as examples:
                    for i, (image, labels) in examples:
                        ...
                        with open(
                            os.path.join(event_directory, filename), "wb"
                        ) as f:
                        ...
                        annotations[os.path.join(event_directory, filename)] = (
                            [
                                {
                                    "centerX": float(
                                        labels[1] * image_shape[1]
                                    ),
                                    "centerY": float(
                                        labels[2] * image_shape[0]
                                    ),
