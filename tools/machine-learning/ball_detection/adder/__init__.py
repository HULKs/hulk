...
app = flask.Flask(
    __name__, static_folder="static", static_url_path="/adder/static"
)
    ...
    return flask.jsonify(
        {
            ...
            "existingAnnotationColor": app.config["arguments"][
                "existing_annotation_color"
            ],
            "addedAnnotationColor": app.config["arguments"][
                "added_annotation_color"
            ],
        }
    )
...
def image_size(index):
    ...
    if index >= len(app.config["existing_annotations"].keys()):
        return flask.make_response(
            "index cannot be larger than amount of paths", 404
        )
...
def telemetry(id):
    ...
    with open(
        f"{app.config['arguments']['output_annotations_file']}.{id}.telemetry",
        "a",
    ) as f:
...
@click.option(
    "--existing-annotation-color",
    default=(0, 0, 0),
    help="Color of existing annotations (three uint8 color components in [0,255] in RGB)",
    show_default=True,
)
@click.option(
    "--added-annotation-color",
    default=(0, 255, 0),
    help="Color of added annotations by this tool (three uint8 color components in [0,255] in RGB)",
    show_default=True,
)
