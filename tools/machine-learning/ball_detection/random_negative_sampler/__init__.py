...
def convert_to_ycbcr(config, image):
    if config["image_color_space"] == "YCbCr":
        ...
        config["image_color_space"] == "RGB"
        or config["image_color_space"] == "Grayscale"
    ):
...
def replace_circles_with_gray(config, image, circles):
    for circle in circles:
        ...
        maximum_x = min(
            image.shape[1], math.ceil(circle["centerX"] + circle["radius"])
        )
        minimum_y = max(0, math.floor(circle["centerY"] - circle["radius"]))
        maximum_y = min(
            image.shape[0], math.ceil(circle["centerY"] + circle["radius"])
        )
...
def generate_random_sample(config, random_generator, image):
    size = int(
        random_generator.uniform(
            config["minimum_sample_size_in_image"],
            config["maximum_sample_size_in_image"],
        )
    )
    ...
    if config["sample_color_space"] == "RGB":
        ...
        sample_image = sample_image[
            ..., 0
        ]  # TODO: Refactor to use (32, 32, 3) for grayscale (reconsider JPEG defaults)
...
def write_sample_to_tfrecord(config, sample_image):
    components = 0
    if (
        config["sample_color_space"] == "YCbCr"
        or config["sample_color_space"] == "RGB"
    ):
        components = 3
    elif config["sample_color_space"] == "Grayscale":
        components = 1
    ...
    return tfrecord.serialize(
        data_shape=(config["sample_size"], config["sample_size"], components),
...
def amount_of_segments_in(config, image, positions_x, positions_y):
    ...
    for x, y in zip(positions_x, positions_y):
        ...
        current_segment_color = (
            current_segment_accumulated_color / current_segment_amount
            if current_segment_amount != 0
            else color
        )
        if (
            abs(color - current_segment_color)
            < config["segments_color_threshold"]
        ):
...
def collect_segments_samples(config, random_generator, image):
    if config["sample_color_space"] != "Grayscale":
        raise RuntimeError(
            f"collect_segments_samples not implemented for sample color space {config['sample_color_space']}"
        )
    ...
    for _ in range(
        int(
            config["segments_amount"]
            * config["maximum_attempts_factor_per_image"]
        )
    ):
        ...
        amount_of_horizontal_segments = sum(
            [
                amount_of_segments_in(
                    config,
                    sample_image,
                    range(config["sample_size"]),
        ...
        amount_of_vertical_segments = sum(
            [
                amount_of_segments_in(
                    config,
                    sample_image,
                    [x] * config["sample_size"],
                    range(config["sample_size"]),
                )
                for x in range(config["sample_size"])
            ]
        )
        amount_of_segments = max(
            amount_of_horizontal_segments, amount_of_vertical_segments
        )

        if amount_of_segments > config["segments_lower_bound_amount"]:
            valid_samples_found += 1
            tfrecord_examples.append(
                write_sample_to_tfrecord(config, sample_image)
            )
...
def collect_jpeg_samples(config, random_generator, image):
    ...
    tfrecord_examples = []
    for _ in range(
        int(config["jpeg_amount"] * config["maximum_attempts_factor_per_image"])
    ):
...
def collect_sobel_samples(config, random_generator, image):
    ...
    for _ in range(
        int(
            config["sobel_amount"] * config["maximum_attempts_factor_per_image"]
        )
    ):
        ...
        grad_x = cv2.Sobel(
            ...
            dx=1,
            dy=0,
            ksize=config["sobel_filter_kernel_size"],
        ...
        sobel_image = cv2.addWeighted(
            src1=abs_grad_x, alpha=0.5, src2=abs_grad_y, beta=0.5, gamma=0
        )
...
def collect_entropy_samples(config, random_generator, image):
    ...
    for _ in range(
        ...
        image_blurred = cv2.GaussianBlur(
            src=sample_image,
            ksize=(
                config["entropy_blur_kernel_size"],
                config["entropy_blur_kernel_size"],
            ),
            sigmaX=0,
            sigmaY=0,
...
@click.option(
    "--image-random-seed",
    default=42,
    help="Random seed for images",
    show_default=True,
)
@click.option(
    "--minimum-sample-size-in-image",
    default=16,
    help="Minimum sample size in image",
    show_default=True,
)
@click.option(
    "--maximum-sample-size-in-image",
    default=240,
    help="Maximum sample size in image",
    show_default=True,
)
@click.option(
    "--sample-size", default=32, help="Sample size", show_default=True
)
@click.option(
    "--image-color-space",
    type=click.Choice(["YCbCr", "RGB", "Grayscale"], case_sensitive=False),
    default="YCbCr",
    help="The color space of source images",
    show_default=True,
)
@click.option(
    "--sample-color-space",
    type=click.Choice(["YCbCr", "RGB", "Grayscale"], case_sensitive=False),
    default="Grayscale",
    help="The color space of source samples",
    show_default=True,
)
@click.option(
    "--default-gray",
    default=128,
    help="Default gray (uint8 Y component in [0,255])",
    show_default=True,
)
@click.option(
    "--maximum-attempts-factor-per-image",
    default=3,
    help="The maximum attempts factor per method",
    show_default=True,
)
@click.option(
    "--segments-amount",
    default=5,
    help="Amount of samples to generate with segments method",
    show_default=True,
)
@click.option(
    "--segments-color-threshold",
    default=60,
    help="Threshold for segment clustering",
    show_default=True,
)
@click.option(
    "--segments-lower-bound-amount",
    default=60,
    help="Required lower bound of amount of segments",
    show_default=True,
)
@click.option(
    "--jpeg-amount",
    default=5,
    help="Amount of samples to generate with JPEG method",
    show_default=True,
)
@click.option(
    "--jpeg-quality", default=50, help="JPEG quality", show_default=True
)
@click.option(
    "--jpeg-lower-bound-size",
    default=500,
    help="Required lower bound of JPEG compressed size",
    show_default=True,
)
@click.option(
    "--sobel-amount",
    default=5,
    help="Amount of samples to generate with sobel method",
    show_default=True,
)
@click.option(
    "--sobel-blur-kernel-size",
    default=5,
    help="Gaussian kernel size (odd numbers required)",
    show_default=True,
)
@click.option(
    "--sobel-filter-kernel-size",
    default=3,
    help="Sobel filter kernel size (odd numbers required)",
    show_default=True,
)
@click.option(
    "--sobel-scale", default=1, help="Sobel filter scale", show_default=True
)
@click.option(
    "--sobel-lower-bound-sum",
    default=30000,
    help="Required lower bound of sum of colors in edge detected image",
    show_default=True,
)
@click.option(
    "--entropy-amount",
    default=5,
    help="Amount of samples to generate with entropy method",
    show_default=True,
)
@click.option(
    "--entropy-blur-kernel-size",
    default=5,
    help="Gaussian kernel size (odd numbers required)",
    show_default=True,
)
@click.option(
    "--entropy-lower-bound",
    default=6,
    help="Required lower bound of entropy",
    show_default=True,
)
    ...
    with click.progressbar(annotations.items()) as annotation_items:
        for image_path, circles in annotation_items:
            try:
                ...
                tfrecord_examples += collect_segments_samples(
                    config, random_generator, image
                )
                tfrecord_examples += collect_jpeg_samples(
                    config, random_generator, image
                )
                tfrecord_examples += collect_sobel_samples(
                    config, random_generator, image
                )
                tfrecord_examples += collect_entropy_samples(
                    config, random_generator, image
                )
            except TypeError:
