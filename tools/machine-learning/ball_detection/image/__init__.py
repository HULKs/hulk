...
def size(path: str):
    ...
    with open(path, "rb") as f:
        head = f.read(24)
        if len(head) != 24:
            raise RuntimeError(f"Failed to get size of {path}: len(head) != 24")
...
def convert_color_space(
    image: np.ndarray, source_color_space: str, target_color_space: str
):
    ...
    if source_color_space == "YCbCr":
        if target_color_space == "YCbCr":
            ...
            return cv2.cvtColor(
                cv2.cvtColor(image[:, :, [2, 0, 1]], cv2.COLOR_YCrCb2BGR),
                cv2.COLOR_BGR2GRAY,
            )
        else:
            raise RuntimeError(
...
def crop(
    ...
    if (
        border_top_left[0, 0] > 0
        or border_top_left[1, 0] > 0
        or border_bottom_right[0, 0] > 0
        or border_bottom_right[1, 0] > 0
    ):
    ...
    # actually crop image
    return image[
        upper_left[1, 0] : lower_right[1, 0],
        upper_left[0, 0] : lower_right[0, 0],
    ]


def resize(
    image: np.ndarray, size: np.ndarray, interpolation=cv2.INTER_NEAREST
):
    """size with shape (2, 1)"""
    return cv2.resize(
        image, (size[0, 0], size[1, 0]), interpolation=interpolation
    )
