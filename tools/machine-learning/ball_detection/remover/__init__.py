...
app = flask.Flask(
    __name__, static_folder="static", static_url_path="/remover/static"
)
...
def image(index):
    ...
    if index >= len(app.config["annotations"].keys()):
        return flask.make_response(
            "index cannot be larger than amount of images", 404
        )
    ...
    if "crop" in flask.request.args:
        try:
            ...
            return flask.make_response(
                "centerX, centerY, or radius missing or malformed", 404
            )

        upper_left = np.array([[center_x - radius], [center_y - radius]])
        lower_right = np.array([[center_x + radius], [center_y + radius]])
    ...
    if "scale" in flask.request.args:
        try:
            scale_width = (
                int(flask.request.args["width"])
                if "width" in flask.request.args
                else None
            )
            scale_height = (
                int(flask.request.args["height"])
        ...
        size = np.array([[scale_width], [scale_height]])
...
def write_output_annotation_files():
    ...
    for annotation_index in app.config["annotation_indices"]:
        ...
        output_annotations = (
            removed_annotations
            if annotation_index["removed"]
            else remaining_annotations
        )
        output_annotations[image_path].append(circle)

    # write files
    with open(
        app.config["arguments"]["output_remaining_annotations_file"], "w"
    ) as f:
        json.dump(remaining_annotations, f, sort_keys=True, indent=4)
        f.write("\n")
    with open(
        app.config["arguments"]["output_removed_annotations_file"], "w"
    ) as f:
...
def toggle_removed(index):
    ...
    if index >= len(app.config["annotation_indices"]):
        return flask.make_response(
            "index cannot be larger than amount of annotations", 404
        )

    # toggle removed state
    app.config["annotation_indices"][index]["removed"] = not app.config[
        "annotation_indices"
    ][index]["removed"]
    ...
    return flask.jsonify(
        {"removed": app.config["annotation_indices"][index]["removed"]}
    )
...
def telemetry(id):
    ...
    with open(
        f"{app.config['arguments']['output_remaining_annotations_file']}.{id}.telemetry",
        "a",
    ) as f:
...
@click.option(
    "--debug",
    is_flag=True,
    help="Run server in debug/development mode which enables hot reloading of the application",
)
@click.option(
    "--host",
    default="localhost",
    help="Hostname to listen on, set this to '0.0.0.0' to have the server available externally as well",
    show_default=True,
)
@click.option(
    "--port", default=5000, help="Port of the webserver", show_default=True
)
@click.option(
    "--crop-scale-factor",
    default=1.5,
    help="Scale factor when cropping annotations",
    show_default=True,
)
@click.option(
    "--color-space",
    ...
    default="YCbCr",
    help="Color space of raw images",
    show_default=True,
)
@click.option(
    "--grid-image-size",
    default=200,
    help="Size of scaled images in grid",
    show_default=True,
)
@click.option(
    "--default-gray",
...
@click.argument(
    "output_remaining_annotations_file", type=click.Path(dir_okay=False)
)
@click.argument(
    "output_removed_annotations_file", type=click.Path(dir_okay=False)
)
def server(*args, **kwargs):
    ...
    try:
        with open(
            app.config["arguments"]["output_removed_annotations_file"]
        ) as f:
    ...
    def is_removed(image, circle_index):
        ...
        for removed_circle in removed_annotations[image]:
            if (
                removed_circle["centerX"] == existing_circle["centerX"]
                and removed_circle["centerY"] == existing_circle["centerY"]
                and removed_circle["radius"] == existing_circle["radius"]
            ):
                return True
        return False
    ...
    app.config["annotation_indices"] = [
        ...
        for image_index, image in enumerate(
            sorted(app.config["annotations"].keys())
        )
    ...
    app.run(
        debug=app.config["arguments"]["debug"],
        host=app.config["arguments"]["host"],
        port=app.config["arguments"]["port"],
    )
