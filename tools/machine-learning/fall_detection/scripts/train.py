...
def train(
    ...
    for label_shift, (train_input_data, data_labels) in enumerate(
        ...
        if label_shift < number_of_label_shifts - 2:
            continue
        ...
        with wandb.init(
            ...
            unbalanced_test_labels = balanced_test_dataframe.select(
                cs.starts_with("labels")
                & cs.ends_with("_" + str(run.config["label_shift"]))
            )
            balanced_test_labels = do_class_balancing(
                unbalanced_test_labels, run.config["label_shift"]
            )
