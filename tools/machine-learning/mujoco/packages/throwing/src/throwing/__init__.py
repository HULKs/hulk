...
def _calculate_initial_velocity(start: np.ndarray, target: np.ndarray, acceleration: np.ndarray, time_to_reach: float) -> np.ndarray:
    ...
    initial_velocity = (displacement - 0.5 * acceleration *
                        time_to_reach ** 2) / time_to_reach
...
class ThrowableObject:
    def __init__(self, model: mujoco.MjModel, data: mujoco.MjData, plane_body: str, throwable_body: str):
        self.model = model
        self.data = data
    ...
    def throw_has_ended(self) -> bool:
        ...
        """
        geoms = (self.model.body_geomadr[self.throwable_index],
                 self.model.body_geomadr[self.ground_index])
        for i in range(self.data.ncon):
            contact = self.data.contact[i]
            if (contact.geom1, contact.geom2) == geoms or (contact.geom2, contact.geom1) == geoms:
                return True
        return False
    ...
    def random_throw(self, target: np.ndarray, *, time_to_reach: float, distance: float):
        ...
        qvel_index = self.model.jnt_dofadr[self.model.body_jntadr[self.throwable_index]]
